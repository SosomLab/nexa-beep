...
2. **결정 없이 진행 가능한 코드** — 🔴 **M4-9 전송 완료 종단 ack + 종료 가드**(P0 — 발신 "완료"가 소켓 쓰기 시점일 뿐 · 08-10 점검) · **M1 잔여**(M1-2 L1 링크 구독 3-OS · M1-8 S4 유니캐스트 · M1-9 링크로컬) · **M1-11 와이어 평문 자동 회귀**(CI 상시 — SEC-1의 잔여 절반) · **M4 잔여**(검사(§6) 어댑터 — 지금은 `Unavailable` 고정 · 수락 후 취소 UX · 폴더 전송 · M4-5 imgdec 분리) · **M3 잔여**(M3-1b OS 동작 어댑터 — mac/linux 클립보드 포함 · M3-2 트레이 · M3-6 SAS 대조 UX — 60자리는 구현됐는데 **출력 지점이 없다** · M3-16 주소 모달 · M2-8 잔여 = 행 스피너·⌘K 워커 이관).
