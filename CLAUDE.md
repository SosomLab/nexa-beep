...
- 현 단계: **설계 문서 33종(00~32) · ADR 13종 · DR-1~28 · FR 119건+ · 리스크 R-1~21.** 코드는 **M0 기반 🚧(크기 축 ✅ · RSS = ★Windows 17.1MB 예산 안쪽 첫 관측(08-12) · mac 85.7MB 재실측 잔여) → M1 발견 🚧(S1~S3 실물·와이어 확정 · ★R-21 ✅=M1-4b · ★R-19 ✅=M1-10 표시 이름 정제 · 잔여 M1-2·M1-8·M1-9·M1-11·M1-13) → M2 대화 🚧(실물 Noise·M2-8 연결 워커 ✅ · ★**M2-5a 신원·핀 영속 ✅ = R-17 해소**(D-18 §3 확정 · 백업/복원 핫 로딩) · 잔여 M2-4b·M2-5b) → M3 셸·UI 🚧(말풍선 대화 화면 · 커스텀 컨트롤 **17종**(+Switch·크기 배율) · **★M3-15 설정 영속 ✅**(D-25 · `crates/nexa-conf`) · **★M3-17 프로필 ✅ 일습** — 화면·공개 토글·백업/복원·**교환 와이어**(세션 경유·프리페치)·**목록 아바타 2줄 행·프로필 보기 카드**·아바타 실사진 · ★**Windows 한/영 직접 토글 ✅**(27 §8 — IME 없이 앱이 모드 소유 · 조합 Enter=확정+전송 동시 확정) · 잔여 M3-2 트레이·M3-6 SAS UX) → M4 전송·무해화 🚧(파일 송수신 ✅ · ★M4-9 종단 ack ✅ · **Win MotW ✅**(mac·Win 2/3-OS) · **★M4-5 imgdec ✅ 코어+포장 동봉+수신 미리보기+★디코드 워커화**(UI 무정지 — 08-13) · ★**대량 전송 Backpressure 해소 ✅**(08-13 — 스트림별 폴링이 2MiB에서 세션을 끊던 것 · `MuxSession::recv_any`) — **잔여 = 권한 강등 하나**(그때 R-5 해소) · M4-10 전송 동기·재개 설계 대기) → M5 배포(★**v0.1.6 공개** — 5타깃×2채널·자산 14종 · M5-4b = choco/winget 변수만(보류 중) · **그룹 M5-1 미착수**)**(08-13 · **534테스트 green** · ★2-PC 실증: Win↔Mac IP 수동 등록·메시지·파일 왕복 · ★**무인자 = 창+실물 기본**·`--help`(+★**GUI 실행 가능 세션 판정** — 못 여는 자리면 안내 후 exit 3 · 3-OS) · dedup 세션 리셋+스레드 대피(재대화 연속성)).
...
## 5. 다음 단계 (2026-08-13 · STATUS 08-13 13차 기준)
...
2. **결정 없이 진행 가능한 코드**(열린 P0 없음 — 08-12 기준) — **M4-5ⓒ 권한 강등**(잡오브젝트/seccomp — 끝내면 **R-5 해소** · ⓐ 포장 동봉·ⓑ 수신 미리보기는 완료) · **M1 잔여**(M1-2 L1 링크 구독 3-OS · M1-8 S4 유니캐스트 · M1-9 링크로컬 · **M1-13 발견 호환성** 설계 등록분) · **M1-11 와이어 평문 자동 회귀**(CI 상시 — SEC-1의 잔여 절반) · **M4 잔여**(검사(§6) 어댑터 — 지금은 `Unavailable` 고정 · 수락 후 취소 UX · 폴더 전송 · M4-4 zip 파서 어댑터) · **M3 잔여**(M3-1b — 클립보드·컨텍스트 메뉴 ✅, 잔여는 `StdAccel`·스크롤 관성·창 닫기 관례 · **M3-2 트레이 = 🔴 트레이 아이콘 의존 판단 대기**(아이콘 없이 창만 숨기면 앱을 되찾을 수 없다) · **M3-6 SAS 대조 UX — 60자리는 구현됐는데 출력 지점이 없다**) · **M5-1 그룹**(v1 핵심인데 미착수).
