...
- 현 단계: **설계 문서 31종(00~30) · ADR 11종 · DR-1~28 · FR 119건+ · 리스크 R-1~21.** 코드는 **M0 기반 🚧(크기 축 ✅·RSS 실기 잔여) → M1 발견 🚧(S1~S3 실물·와이어 확정 · ★R-21 주소 계열 비대칭 ✅ 해소=M1-4b) → M2 대화 🚧(실물 Noise·양방향 GUI 실증 · ★M2-8 연결 워커 ✅ — 죽은 상대 클릭 무정지 · 잔여 M2-4b·M2-5) → M3 셸·UI 🚧(★대화 화면 말풍선 개편 완료 — 좌우 풍선·꼬리·타임스탬프·날짜 알약·멀티라인 입력·**클립보드 3-OS 실물**·**우클릭 컨텍스트 메뉴**·커스텀 컨트롤 **16종**·글리프 폴백·광학 크기 보정 · **설정 2단 계층+고정 밴드**) → M4 전송·무해화 🚧(파일 송수신 ✅ 스레드 기록화 · 격리함 채널 분리·출처·비우기 · ★잔여 P0 = M4-9 전송 완료 종단 ack)**(08-11 · **430테스트 green** · ★ **v0.1.2 첫 배포 공개** — 5타깃 설치본+포터블 · Homebrew/winget/choco).
  ...
  **다음 관문(전부 🔴 사용자 확정)** — **D-23**(ADR-0010 등급·알림) · **D-24**(기능 범위 v1.0 커트라인) · **D-18 §3**(기록 암호화 키 관리 — 🔴 R-17 TOFU 핀 비영속을 잠근다) · **Q-29-1**(M1-10 — 🔴 R-19 실명 평문 방송). ~~D-22~~·~~D-25~~는 **✅ 확정·구현 완료**(D-22 08-08 · D-25 08-11 — `crates/nexa-conf`+M3-15 설정 영속).
