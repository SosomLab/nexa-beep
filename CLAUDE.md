...
- 현 단계: **설계 문서 31종(00~30) · ADR 11종 · DR-1~28 · FR 119건+ · 리스크 R-1~21.** 코드는 **M0 기반 🚧(크기 축 ✅·RSS 실기 잔여) → M1 발견 🚧(S1~S3 실물·와이어 확정 · ★R-21 ✅=M1-4b · ★R-19 ✅=M1-10 표시 이름 정제) → M2 대화 🚧(실물 Noise·M2-8 연결 워커 ✅ · ★**M2-5a 신원·핀 영속 ✅ = R-17 해소**(D-18 §3 확정 · 백업/복원 핫 로딩) · 잔여 M2-4b·M2-5b) → M3 셸·UI 🚧(말풍선 대화 화면 · 커스텀 컨트롤 **17종**(+Switch·크기 배율) · **★M3-15 설정 영속 ✅**(D-25 · `crates/nexa-conf`) · **★M3-17 프로필 ✅ 골격 일습** — 화면·공개 토글·백업/복원·**교환 와이어**(세션 경유·프리페치)·**목록 아바타 2줄 행·프로필 보기 카드** · 설정 desc 워드랩) → M4 전송·무해화 🚧(파일 송수신 ✅ · ★M4-9 종단 ack ✅ · **Win MotW ✅** · **★M4-5 imgdec 코어 ✅**(격리 디코드 — 아바타 실사진 · 잔여 = 포장 동봉·수신 미리보기·권한 강등) · M4-10 전송 동기·재개 설계 대기) → M5 배포(★**v0.1.3 공개** · M5-4b = choco/winget 변수만(보류 중))**(08-11 · **472테스트 green**).
...
5. 🔐 **보안은 상시** — 트리거·트래커 = [TODO §9](docs/TODO.md) SEC-T1~T9 · 열린 항목 12건(최우선 R-18·R-17 — ~~R-19~~ 해소 08-11). 기준 = [29](docs/29-wire-security-audit.md) W-1~W-12.
