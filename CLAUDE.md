...
- 현 단계: **설계 문서 31종(00~30) · ADR 11종 · DR-1~28 · FR 119건+ · 리스크 R-1~20.** 코드는 **M0 기반 🚧(크기 축 ✅·RSS 실기 잔여) → M1 발견 🚧(S1~S3 실물·와이어 확정) → M2 대화 🚧(실물 Noise·양방향 GUI 실증 — M2-4b·M2-5 잔여) → M3 셸·UI 🚧 → M4 전송·무해화 🚧(파일 송수신·격리·승인 UX·대역폭·안전 회귀 CI 완성)**(08-10 · **365테스트 green — Windows 실측**).
...
2. **결정 없이 진행 가능한 코드** — **M1 잔여**(M1-2 L1 링크 구독 3-OS · M1-8 S4 유니캐스트 · M1-9 링크로컬) · **M1-11 와이어 평문 자동 회귀**(CI 상시 — SEC-1의 잔여 절반) · **M4 잔여**(검사(§6) 어댑터 — 지금은 `Unavailable` 고정 · 수락 후 취소 UX · 폴더 전송 · M4-5 imgdec 분리) · **M3 잔여**(M3-1b OS 동작 어댑터 · M3-2 트레이 · M3-6 SAS 대조 UX — 60자리는 구현됐는데 **출력 지점이 없다**).
