...
## 5. 다음 단계 (2026-08-13 · STATUS 08-13 7차 기준)

> **08-13 하루 요약** — ★**v0.1.5 공개**(imgdec 픽셀 상한 4096²·세션 기본 포트 47200 일습) → **2-PC 실증 성공**(Win↔Mac IP 수동 등록·메시지·파일 왕복) → 실기 피드백 일습: 비발견 상대 목록 유지(`extra_peers`+수동 주소 재연결) · 인바운드 자동 열림 제거 · 읽지 않음 배지 · `--port` 인자 · 텍스트 에디터(클립보드 전 뷰·가로 스크롤) · kitty 프로토콜 누수 3중 방어 · **상태 점 보강**(TCP keepalive — FIN 없는 죽음 22초 감지 실측 · 연결 실패=빨강 · 유령 행 정리) · 파일 전송 자격 **경고 모달** · **ⓑ 자동 재연결 백오프**(5s→5분·수동 우선) · **[13 §12-1](docs/13-code-design-standards.md) 자동·반복 동작 점검 규칙 신설** · mux 백프레셔 2MiB 수정 · 26번 전면 재작성(상주 빌더). **498 green**.
...
3. ★ **이 저장소는 Windows 실기 항목이 열려 있다**(Windows PC에서 작업 시 우선 후보) — ~~Win MotW 표식 어댑터~~(**✅ 08-11 완료** — Zone.Identifier ADS·ZoneId=3·종단 실측) · **AMSI 검사**(FR-S-15) · **Windows 콘솔 핸들러**(R-16 잔여) · Windows IME·방화벽 실기 검증.
