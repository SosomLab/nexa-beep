...
impl Msg {
    ...
    const fn row(self) -> [&'static str; 4] {
        match self {
            ...
            Msg::ServerTypeDesc => [
                "Auto = provided by the server during handshake (recommended). Selectable only in Managed mode.",
                "자동 = 핸드셰이크 때 서버가 제공(권장). 관리형 모드에서만 직접 선택할 수 있습니다.",
                "自动 = 握手时由服务器提供（推荐）。仅托管模式可选。",
                "自動 = ハンドシェイク時にサーバーが提供（推奨）。管理モードのみ選択可。",
            ],
            Msg::ServerTypeAuto => ["Auto (server-provided)", "자동 (서버 제공)", "自动（服务器提供）", "自動（サーバー提供）"],
            Msg::ServerTypeGateway => ["Gateway", "게이트웨이", "网关", "ゲートウェイ"],
            Msg::ServerTypeRelay => ["Relay", "릴레이", "中继", "リレー"],
            Msg::ServerTypeContent => ["Content", "컨텐츠", "内容", "コンテンツ"],
            Msg::ServerTypeRegistered => ["Registered", "등록 전용", "注册制", "登録制"],
