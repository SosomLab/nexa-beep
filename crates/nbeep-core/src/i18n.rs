...
impl Msg {
    ...
    const fn row(self) -> [&'static str; 4] {
        match self {
            ...
            Msg::UserSuggest => ["Suggest a passphrase", "암호 자동 생성", "自动生成口令", "パスフレーズを自動生成"],
            Msg::UserSuggestDesc => ["Generates a random 12-character passphrase (xxxx-xxxx-xxxx) and puts it in the field above.", "무작위 12자 암호(xxxx-xxxx-xxxx)를 만들어 위 칸에 넣습니다.", "生成随机 12 位口令（xxxx-xxxx-xxxx）并填入上方。", "ランダムな 12 文字（xxxx-xxxx-xxxx）を生成して上の欄に入れます。"],
            Msg::UserSuggestVerb => ["Generate", "생성", "生成", "生成"],
