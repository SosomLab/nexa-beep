...
pub enum Msg {
    ...
    UserTestDesc,
    UserTestVerb,
    /// 사용자 키 교체(기기 분실 · ADR-0015 §3-5 Succession — S2-f).
    UserRotate,
    UserRotateDesc,
    UserRotateVerb,
    StUserRotateArm,
    StfUserRotated,
    /// 행 노트 카운트다운(09-06 — 설정 창 안에서 바로 보이게): 생성 무장 · 교체 무장.
    StfNoteRegenArm,
    StfNoteRotateArm,
    CardSuccessionConflict,
    StUserNeedBoth,
    StfUserTesting,
    StfUserTestOk,
    StfUserTestFail,
...
impl Msg {
    ...
    const fn row(self) -> [&'static str; 4] {
        match self {
            ...
            Msg::CatUser => ["User", "사용자", "用户", "ユーザー"],
            Msg::UserEnabled => ["Multi-device identity", "다중 기기 신원", "多设备身份", "マルチデバイス ID"],
            Msg::UserEnabledDesc => ["Link this PC with your other devices under one user. Off = this PC is a standalone node (default). Turning on fills empty fields with defaults and verifies at once.", "이 PC를 내 다른 기기와 한 사용자로 묶습니다. 끄면 이 PC는 단독 노드(기본). 켜면 빈 칸을 기본값으로 채우고 바로 검증합니다.", "将此电脑与您的其他设备绑定为同一用户。关闭 = 独立节点（默认）。开启时自动填充空白项并立即验证。", "この PC を他のデバイスと同じユーザーに束ねます。オフ = 単独ノード（既定）。オンにすると空欄を既定値で埋めて直ちに検証します。"],
            Msg::UserHandle => ["Handle", "핸들(내 아이디)", "我的 ID（句柄）", "ハンドル"],
            Msg::UserHandleDesc => ["Short public name shared by your devices (3–32 chars: letters, digits . _ - @). A label only — not your identity.", "내 기기들이 공유하는 짧은 공개 이름(3~32자 · 영숫자 . _ - @). 라벨일 뿐 신원이 아닙니다.", "您的设备共用的简短公开名称（3–32 字符：字母、数字 . _ - @）。仅为标签，不是身份。", "デバイス間で共有する短い公開名（3〜32 文字 · 英数字 . _ - @）。ラベルであり ID ではありません。"],
            Msg::UserPass => ["Pairing passphrase", "페어링 암호", "配对口令", "ペアリングパスフレーズ"],
            Msg::UserPassDesc => ["Secret shared only by your devices — derives the meeting point and proves membership. Never sent to any server. Stored sealed on this PC. Changing it does not change your identity.", "내 기기끼리만 아는 비밀 — 만남 지점을 만들고 소속을 증명합니다. 어떤 서버로도 가지 않고 이 PC에 봉인 저장됩니다. 바꿔도 신원은 그대로입니다.", "仅您的设备共享的秘密 — 生成会合点并证明归属。绝不发送到任何服务器，在本机加密保存。更改后身份不变。", "自分のデバイス間だけの秘密 — 待ち合わせ地点を作り所属を証明します。サーバーには送られず、この PC に封印保存されます。変更しても ID は変わりません。"],
            Msg::UserTest => ["Verify", "인증 테스트", "验证", "認証テスト"],
            Msg::UserTestDesc => ["Derives your user identity from the values above and opens (or creates) this PC's sealed user key. Success is remembered; any change to the values requires verifying again.", "위 값으로 사용자 신원을 파생하고 이 PC의 봉인된 사용자 키를 열거나(없으면) 만듭니다. 성공은 저장되며, 값을 바꾸면 다시 인증해야 합니다.", "根据上方的值派生用户身份并打开（或创建）本机加密的用户密钥。成功会被记住；值一旦更改需重新验证。", "上の値からユーザー ID を導出し、この PC の封印ユーザーキーを開く（なければ作る）。成功は保存され、値を変えると再認証が必要です。"],
            Msg::UserTestVerb => ["Verify", "인증", "验证", "認証"],
            Msg::UserRotate => ["Replace user key (lost device)", "사용자 키 교체 (기기 분실)", "更换用户密钥（设备丢失）", "ユーザー鍵の交換（デバイス紛失）"],
            Msg::UserRotateDesc => ["Creates a new user key on this PC and signs a succession with the old one. Other devices are revoked — change the pairing passphrase, then enter the new one on each device you still own to pair again. Contacts automatically carry your old ID over to the new one — no re-approval needed.", "이 PC에 새 사용자 키를 만들고 옛 키로 후계를 서명합니다. 다른 기기는 전부 폐기됩니다 — 페어링 암호를 바꾼 뒤, 아직 가진 기기마다 새 암호를 넣어 다시 결합하세요. 상대에게는 옛 ID가 새 ID로 자동으로 이어집니다.", "在本机创建新的用户密钥并用旧密钥签署继承。其他设备全部撤销 — 请更改配对口令，然后在仍拥有的每台设备上输入新口令重新配对。联系人会自动把旧 ID 延续到新 ID，无需重新批准。", "この PC に新しいユーザー鍵を作り、旧鍵で継承に署名します。他のデバイスはすべて失効します — ペアリング合言葉を変え、手元に残る各デバイスで新しい合言葉を入力して再結合してください。相手側では旧 ID が新 ID に自動で引き継がれ、再承認は不要です。"],
            Msg::UserRotateVerb => ["Replace", "교체", "更换", "交換"],
            Msg::StUserRotateArm => ["Press Replace again within 5 s — all other devices will be revoked (pair again with a new passphrase)", "5초 안에 교체를 한 번 더 누르면 실행 — 다른 기기는 전부 폐기됩니다(새 암호로 다시 결합)", "5 秒内再次按“更换”即执行 — 其他设备将全部撤销（用新口令重新配对）", "5秒以内にもう一度「交換」を押すと実行 — 他のデバイスはすべて失効します（新しい合言葉で再結合）"],
            Msg::StfNoteRegenArm => ["Press the regenerate icon again within {} s to replace the passphrase (red = armed)", "{}초 안에 생성 아이콘을 한 번 더 누르면 새 암호로 바뀝니다(빨강 = 대기 중)", "{} 秒内再次按生成图标即替换口令（红色 = 待确认）", "{}秒以内にもう一度生成アイコンを押すと合言葉が置き換わります（赤 = 待機中）"],
            Msg::StfNoteRotateArm => ["Press Replace again within {} s to run — all other devices will be revoked (red = armed)", "{}초 안에 교체를 한 번 더 누르면 실행됩니다 — 다른 기기는 전부 폐기됩니다(빨강 = 대기 중)", "{} 秒内再次按“更换”即执行 — 其他设备将全部撤销（红色 = 待确认）", "{}秒以内にもう一度「交換」を押すと実行 — 他のデバイスはすべて失効します（赤 = 待機中）"],
            Msg::StfUserRotated => ["✓ User key replaced — ID {} → {} · now change the pairing passphrase", "✓ 사용자 키 교체됨 — ID {} → {} · 이제 페어링 암호를 바꾸세요", "✓ 用户密钥已更换 — ID {} → {} · 请立即更改配对口令", "✓ ユーザー鍵を交換 — ID {} → {} · 続けてペアリング合言葉を変えてください"],
            Msg::CardSuccessionConflict => ["⚠ conflicting user-key successions for this user — confirm through another channel", "⚠ 이 사용자의 키 후계가 충돌함 — 다른 채널로 확인하세요", "⚠ 该用户的密钥继承存在冲突 — 请通过其他渠道确认", "⚠ このユーザーの鍵継承が衝突 — 別の経路で確認してください"],
            Msg::StUserNeedBoth => ["Handle and pairing passphrase are both required", "핸들과 페어링 암호가 모두 필요합니다", "需要同时填写句柄和配对口令", "ハンドルとペアリングパスフレーズの両方が必要です"],
            Msg::StfUserTesting => ["Verifying user {}…", "사용자 인증 중… {}", "正在验证用户 {}…", "ユーザーを認証中… {}"],
            Msg::StfUserTestOk => ["✓ User verified — {} (ID {}) · saved", "✓ 사용자 인증 완료 — {} (ID {}) · 저장됨", "✓ 用户验证完成 — {}（ID {}）· 已保存", "✓ ユーザー認証完了 — {}（ID {}）· 保存済み"],
            Msg::StfUserTestFail => ["User verification failed: {} — user features stopped", "사용자 인증 실패: {} — 사용자 기능 중지", "用户验证失败：{} — 用户功能已停止", "ユーザー認証失敗: {} — ユーザー機能を停止"],
            Msg::StNoteUserOff => ["○ Off — this PC is a standalone node", "○ 꺼짐 — 이 PC는 단독 노드", "○ 关闭 — 本机为独立节点", "○ オフ — この PC は単独ノード"],
            Msg::StNoteUserEmpty => ["Both fields are required — nothing is registered until both are set", "핸들·암호 둘 다 필요 — 둘 다 채우기 전에는 어디에도 등록되지 않습니다", "两项均为必填 — 填满之前不会进行任何注册", "両方の入力が必要 — 揃うまでどこにも登録されません"],
            Msg::StNoteUserBadHandle => ["Handle format: 3–32 chars, letters/digits . _ - @ only", "핸들 형식: 3~32자 · 영숫자 . _ - @ 만", "句柄格式：3–32 字符，仅字母/数字 . _ - @", "ハンドル形式: 3〜32 文字 · 英数字 . _ - @ のみ"],
            Msg::StNoteUserShortPass => ["Passphrase must be at least 8 characters", "암호는 8자 이상이어야 합니다", "口令至少 8 个字符", "パスフレーズは 8 文字以上"],
            Msg::StNoteUserUntested => ["Values changed — user features stopped until you verify", "값이 바뀌었습니다 — 인증할 때까지 사용자 기능 중지", "值已更改 — 验证前用户功能已停止", "値が変わりました — 認証するまでユーザー機能は停止"],
            Msg::StNoteUserTesting => ["Verifying…", "인증 중…", "正在验证…", "認証中…"],
            Msg::StfNoteUserVerified => ["✓ Verified · {} · ID {}", "✓ 인증됨 · {} · ID {}", "✓ 已验证 · {} · ID {}", "✓ 認証済み · {} · ID {}"],
            Msg::StfNoteUserFailed => ["✗ Failed: {} — user features stopped", "✗ 실패: {} — 사용자 기능 중지", "✗ 失败：{} — 用户功能已停止", "✗ 失敗: {} — ユーザー機能を停止"],
            Msg::StfNoteUserStrength => ["Strength: {}", "강도: {}", "强度：{}", "強度: {}"],
            Msg::UserStrengthWeak => ["weak — easy to guess", "약함 — 추측되기 쉽습니다", "弱 — 容易被猜到", "弱い — 推測されやすい"],
            Msg::UserStrengthNormal => ["normal", "보통", "中", "普通"],
            Msg::UserStrengthStrong => ["strong", "강함", "强", "強い"],
