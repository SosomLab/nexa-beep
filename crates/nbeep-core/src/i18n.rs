...
impl Msg {
    ...
    const fn row(self) -> [&'static str; 4] {
        match self {
            ...
            Msg::UserRotateDesc => ["Creates a new user key on this PC and signs a succession with the old one. Other devices are revoked — change the pairing passphrase, then enter the new one on each device you still own to pair again. Contacts fold your old ID into the new one automatically.", "이 PC에 새 사용자 키를 만들고 옛 키로 후계를 서명합니다. 다른 기기는 전부 폐기됩니다 — 페어링 암호를 바꾼 뒤, 아직 가진 기기마다 새 암호를 넣어 다시 결합하세요. 상대는 옛 ID를 새 ID로 자동으로 접습니다.", "在本机创建新的用户密钥并用旧密钥签署继承。其他设备全部撤销 — 请更改配对口令，然后在仍拥有的每台设备上输入新口令重新配对。联系人会自动把旧 ID 并入新 ID。", "この PC に新しいユーザー鍵を作り、旧鍵で継承に署名します。他のデバイスはすべて失効します — ペアリング合言葉を変え、手元に残る各デバイスで新しい合言葉を入力して再結合してください。相手は旧 ID を新 ID に自動で畳みます。"],
