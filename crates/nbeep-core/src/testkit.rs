...
// ── 링크 fake — 채널로 이어진 양방향 Link 한 쌍(세션·전송 테스트용) ──
...
/// 서로 연결된 링크 한 쌍을 만든다 — `a`는 `b_peer`를, `b`는 `a_peer`를 향한다.
///
/// 네트워크 없이 세션·프로토콜을 테스트하는 최소 fake(버퍼 채널이라 송신은 블로킹하지 않는다).
#[must_use]
pub fn duplex(a_peer: PeerId, b_peer: PeerId) -> (DuplexLink, DuplexLink) {
    let (a_tx, a_rx) = channel::<Vec<u8>>(); // a → b
    let (b_tx, b_rx) = channel::<Vec<u8>>(); // b → a
    let a = DuplexLink {
        peer: b_peer,
        tx: a_tx,
        rx: b_rx,
