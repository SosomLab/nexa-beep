...
>
> ★ **이 문서는 같은 LAN에서 자동 발견으로 만나는 경로다.** 발견이 닿지 않는 인터넷 너머에서
> **지문(64hex)만으로 만나는 경로**는 자매 문서 **[42 릴레이 랑데부 종단 설명서](42-relay-rendezvous-walkthrough.md)** 에 있다 —
> 랑데부(회전 RID)·홀펀칭·릴레이 폴백이 다를 뿐, **종단 Noise부터 아래는 이 문서와 같은 코드**다(`Link` 계약 · DR-21).
