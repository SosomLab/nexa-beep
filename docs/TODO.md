...
| **M3-2b** | **macOS 메뉴바**(mac PC 몫): `NSStatusItem` 어댑터(objc 직접 — NSSound 선례) — NSImage(컬러 아바타 · 메뉴바 18~22pt@2x) · `button.toolTip` = 이름 · **좌/우클릭 모두 NSMenu**(맥 관례 — 열기·종료) · **AppKit 메인 스레드 강제**(생성·갱신을 winit 메인 루프에서 — 프록시 이벤트) · ⚠ 맥은 "창 닫기 = 앱 유지"가 이미 OS 관례(빨간 버튼) — 스위치 기본값 차등 여부 = 결정지 | P1 | 중 | M3-2 | ☐ |
