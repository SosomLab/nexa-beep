...
| **M5-4b** | **winget·Chocolatey 실제 게시** — 매니페스트 생성·검증은 통과, **게시만 변수로 잠겨 있다**. ⓐ **Windows 실기 확인 선행**(설치본 설치·제거·포터블 실행 — 아직 못 했다) ⓑ 시크릿(`WINGET_TOKEN`·`CHOCO_API_KEY`) + 변수(`WINGET_PUBLISH`·`CHOCO_PUSH`) 설정 ⓒ `publish-windows-packages` 실행 ⓓ 검수 통과 확인(둘 다 중앙 저장소라 즉시 노출 아님) | P1 | 소 | M5-4 | ☐ (변수만 켜면 되지만 **실기 확인 먼저**) |
