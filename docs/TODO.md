...
| **M3-2b** | **macOS 메뉴바**(mac PC 몫): `NSStatusItem` 어댑터(objc 직접 — NSSound 선례) — NSImage(컬러 아바타 · 메뉴바 18~22pt@2x) · `button.toolTip` = 이름 · **좌/우클릭 모두 NSMenu**(맥 관례 — 열기·종료) · **AppKit 메인 스레드 강제**(생성·갱신을 winit 메인 루프에서 — 프록시 이벤트) · ⚠ 맥은 "창 닫기 = 앱 유지"가 이미 OS 관례(빨간 버튼) — 스위치 기본값 차등 여부 = 결정지 | P1 | 중 | M3-2 | ✅ (**08-15 mac 세션**: `tray.rs::mac` — NSStatusItem(가변 길이)+NSMenu(이름 헤더·열기·종료 · 좌/우 공통) · 액션 = 선언 클래스 `NbeepTrayTarget` 셀렉터 → 프록시 · 아이콘 = RGBA→NSBitmapImageRep→NSImage 18pt(32px 원본 = 레티나 2x) · **메인 스레드 강제 = MainThreadMarker**(밖이면 None/무시 fail-soft) · 상태는 thread_local(NSStatusItem 비Send) · **실측 = 메뉴바 diff: 35px 아이콘 3개(신원별 아바타) 등장/소거 확인** · ⏸ 육안 = 메뉴 클릭(열기/종료)·close_to_tray on 숨김/복원) |
