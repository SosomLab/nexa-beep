...
**크리티컬 패스(08-15 현행)** — ~~M0-1 → M0-1b → SP-1(크기 축)~~ ✅ → ~~D-22 확정~~ ✅ → ~~M1-3 와이어 포맷~~ ✅ → ~~M2-7 비동기 펌프~~ ✅ → ~~M4-9 종단 ack~~ ✅ → ~~D-25 → M3-15 설정 영속~~ ✅ → ~~D-18 §3 → M2-5a 신원·핀 영속~~ ✅ → ~~M5-4 배포 2채널~~ ✅(v0.1.4 공개) → ~~M5-1 그룹 개통~~ ✅(08-13 G1~G3) → ~~M3-1e ② IME 상태기계~~ ✅(08-14 · H-25/26/27 완주) → ~~M4-5ⓒ 권한 강등~~ ✅(08-15 · R-5 종결) → ~~M1 잔여(M1-2·M1-8·M1-9)~~ ✅(08-15 — 발견 폴백 사다리 S1~S4 개통·L1 구독 3-OS) → ~~M5-1 G4~~ ✅(08-15 재동기·승격) → ~~M3-19/14 상태 배지~~ ✅(08-15) → **지금 = 🔴 D-23 확정(M2-4b → M3-8/9/10 연쇄) · 🔴 D-24 범위 확정 · M1-2b 전환 세션 연속성(설계 등록) · M3-2d 트레이 결정지** → M2-5b 기록 저장(D-18 §4~ 대기) → M5-5 24h 누수 → v1.
...
| **M1-2b** | **네트워크 전환 세션 연속성 — 상태 유지**(사용자 요청 08-15: 유선↔무선·유선→유선·무선→무선 전환에도 상태 유지) — 있는 조각: L1 감지(M1-2)·재발견(kick)·TCP keepalive 22초 감지·자동 재연결 백오프ⓑ·스레드 대피(재대화 연속 · DR-26 뷰 분리)·핀=키(DR-28 신원은 경로 무관). **설계 잔여**: ⓐ LinkChanged 시 **활성 세션 능동 생사 확인**(keepalive 22초를 기다리지 않고 즉시 판정 — 끊긴 세션은 Closed 처리) ⓑ **직전 활성·핀 상대 우선 즉시 재연결**(백오프 0단부터 — 스케줄 대기 없이 1회) ⓒ 같은 LAN 안 전환은 경로 등급 불변 = **조용히**(DR-28 "경로가 바뀌면 조용히") ⓓ 실기 매트릭스 4종(유선→무선·무선→유선·유선→유선(포트 이동)·무선→무선(SSID/AP 이동)) — 각각 대화 기록·그룹·미승인 전송 상태 유지 확인 | P1 | 중 | M1-2 | 🚧 (★ **08-15 ⓑ 부분 구현** — LinkChanged 시 **재연결 가속**: 걸린 백오프 스케줄 전부 0단·즉시로 리셋 + 직전 대화하다 끊긴 상대(수단 있는 것만) 조용한 연결 1회 — 전환 후 긴 대기 제거 · 잔여 = ⓐ 활성 세션 능동 생사 확인(keepalive 22초 단축) · ⓒ 경로 등급 조용히(M5-3c와 함께) · ⓓ 실기 매트릭스 4종) |
...
| M1-8 | **S4**(ARP/NDP 이웃 유니캐스트 프로브 — 클라이언트 격리 대응) · E-3 결과 기반 | P0 | 중 | M1-7 | ✅ (08-15 — `nbeep-net::neigh`(Linux `/proc/net/arp` 완성 항목 · mac `arp -an` 스폰 — pbcopy 선례 · Win = WNET-1) · 기동 1회 + 광고 16주기(≈12.8s)마다 서브넷 안 이웃(상한 64) HELLO · ★ **수신측 유니캐스트 Announce 응답**(막힌 망 왕복의 나머지 절반) · `s4_targets` 순수 회귀(서브넷 밖 유출 금지 — FR-S-49) · ⏸ 왕복 실기 = 멀티캐스트 차단 망 2대(E-3 자리) · 타이밍은 E-3 실측 후 재조정) |
| M1-9 | 링크로컬 직결(DHCP 없음) 발견(FR-D-7) · 가상/터널 인터페이스 제외 | P1 | 소 | M1-4 | ✅ (08-15 — `nbeep-net::netif`(getifaddrs · unix) 인터페이스별 멀티캐스트 발신(IP_MULTICAST_IF)+서브넷 지향 브로드캐스트+그룹 조인 · **가상·터널 제외**(utun/wg/docker/awdl… 접두사 — VPN 존재 방송 금지 · R-18 결) · 기본 경로 발신은 안전망 유지 · Windows 어댑터 = **WNET-1**(GetAdaptersAddresses — 빈 목록 폴백 = 종전 동작) · ⏸ 실기 = 2대 직결(169.254) · v6 링크로컬(fe80 스코프별)은 S1 best-effort 지위 유지) |
...
| **M1-11** | **와이어 평문 자동 회귀**(FR-S-51 · [29 §4-1](29-wire-security-audit.md)) — ① 골든 레이아웃 ② 금칙어 스캔 ③ tap 일반화 ④ Debug 마스킹 · CI 상시 | P0 | 중 | M1-3 | ✅ (**08-15**: ① 기존 골든 + **허용 목록 연접 = 바이트 동일** 구조 강제(전 패킷 종류) ② 발견 금칙어 스캔 ③ tap을 금칙어 세트(이메일·전화·한글 본문·설정 키)로 일반화 ④ Identity Debug 개인키 4byte-hex 전수 부재 단언 — 전부 `cargo test`라 3-OS CI 상시. **SEC-1 잔여 절반 해소**) |
...
| **T-relaunch** | **relaunch 스크립트 Windows 보강**(08-15 실기 — `tools/relaunch.sh`의 ① 종료 단계가 Windows 프로세스를 못 잡고(0개 감지 → 구 인스턴스가 exe 잠금 → 빌드 os error 5) ④ 창 카운트도 0 고정. PowerShell 분기(`Get-Process nexa-beep \| Stop-Process` · 창 수 = MainWindowTitle 카운트) 추가 · 수용 기준 = Windows에서 relaunch 1회 실행으로 종료→빌드→3신원→발견 확인까지 경고 없이 통과 | P2 | 소 | — | ☐ |
