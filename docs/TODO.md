...
**크리티컬 패스(08-08 현행)** — ~~M0-1 → M0-1b → SP-1~~ ✅ → **🔴 D-22 확정 → M1-3 와이어 포맷 → M1-2/M1-4 실물 발견 → M2-7 비동기 수신 펌프** → M3 UI 잔여 → M4 무해화 → M5 배포.
...
> **설계에 남은 것**: **ADR-0004·0005·0006·0010 사용자 확정** → `Accepted`. **D-22(복제 대응)는 M1-3 와이어 포맷보다 먼저** 답해야 한다. 확정되면 코드 착수.
...
| M1-3b | **수동 엔드포인트**(DR-19 · [19](19-adr-0006-manual-endpoint.md)) — `Transport::add_endpoint`(발견 우회 IP/호스트명 직접 연결·신원은 핸드셰이크 확정) · bin `--serve`/`--connect` · **맥↔docker-linux IP 대화 실증**(발견 경계 우회). 잔여 = 원격 신뢰 등급(M4 파일 차단)·인바운드 요청 대기·재연결 백오프 | P1 | 중 | M1-4 | 🚧 (08-08 — 핵심+**GUI 수동 추가 UI 완료**: `⌘/Ctrl+K` 주소 입력 오버레이(상태바)→add_endpoint→대화 자동 오픈) |
...
| M3-3 | **IME 완성**(FR-U-7 한/중/일) · 고DPI 배율(FR-U-6) · 다크/라이트(FR-U-5) | P0 | 대 | M3-1d | ☐ |
| M3-4 | 다국어 한/영(FR-U-3) · 키보드 우선 조작·타입어헤드(FR-U-4) | P0 | 중 | M3-1 | ☐ |
...
| M3-13 | 🔴 **정상 종료 경로**(FR-P-7 · R-16) — `plat` **종료 신호 포트**(Unix `sigaction` / Win `SetConsoleCtrlHandler` · DR-21) → 종료 플래그 → **깨우기**(accept 타임아웃·셧다운 소켓) → **① GOODBYE 발신**(FR-D-8 절반) **② 세션·소켓·워커 회수**(NFR-B-6) **③ zeroize**(FR-S-22). ⚠️ 지금은 컨테이너에서 **Ctrl+C가 무시된다**(PID 1 · 실측 `docker stop` 10.26초) — 개발 중에는 `docker run --init`([18 §2-1](18-build-and-test.md)) | P0 | 중 | M1-4 | ☐ (08-08 발견) |
