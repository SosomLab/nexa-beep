...
| M1-3b | **수동 엔드포인트**(DR-19 · [19](19-adr-0006-manual-endpoint.md)) — `Transport::add_endpoint`(발견 우회 IP/호스트명 직접 연결·신원은 핸드셰이크 확정) · bin `--serve`/`--connect` · **맥↔docker-linux IP 대화 실증**(발견 경계 우회). 잔여 = GUI 수동 추가 UI·원격 신뢰 등급(M4 파일 차단)·재연결 백오프 | P1 | 중 | M1-4 | 🚧 (08-08 핵심 완료) |
