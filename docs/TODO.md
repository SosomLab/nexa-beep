...
| **M3-2c** | 🔴 **Linux 트레이 범위 결정**: 표준 = StatusNotifierItem(D-Bus — KDE OK·GNOME 확장 필요·파편화) · 구현 = zbus 의존 체인 필요(DR-5 검토) vs **v1 미지원 폴백**(트레이 없음 → Linux에선 `ui.close_to_tray` 숨김·닫기 = 종료 유지) — **권고 = 폴백**(ADR-0014 D-30c Linux 결정과 같은 결 · M3-8 D-Bus 알림과 묶어 v1.5 재검토) | P2 | 소 | M3-2 | ☐ |
