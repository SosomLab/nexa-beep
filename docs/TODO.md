...
| H-26c | ★ **한글 1byte 첫 타 유실 — 수집·분석 완주 후 단일 처방**([34 §2-8](34-hangul-input-issues.md) · 사용자 확정 08-14 "하나씩") — **1차 수집 완료(08-14 자동 실기)**: `commit="동"` 직후 `123`에서 **"1"의 keydown이 앱에 미도달**(트레이스 원문 — 앱 게이트 밖 · winit-IME 경계 소비). 잔여 = ① 재수집(시나리오 `홍길동123aBc최고다!!!` 완주 — 1차는 후반부가 포커스 강탈로 유실 · 자동 타이핑 중 화면 개입 금지 · 산출물 = 트레이스+단계 캡처 → `scratchpad/ime-collect/`) ② winit 상류 이슈·버전 조사(한국어 IME 확정 직후 키 소실) ③ 처방 택1: winit 갱신 / `nbeep-plat` NSEvent 로컬 모니터 보충 주입(DR-21 이음새) — **수정은 ImeGate(M3-1e) 회귀 박제와 함께 한 번에** | P0 | 중 | M3-1e | 🚧 |
