...
| M1-7a | **D-8a VM 상호운용 검증**(08-08 분리 — 사용자 환경: 맥북 + VM/Docker) — 실물 소켓 경로 · **크로스 OS 와이어 상호운용**(맥↔Win/Linux) · **Windows 방화벽 실동작** · 2노드 종단. 브리지(유선 권장)/호스트 온리. **타이밍 확정은 불가**(가상 스위치 손실≈0) | P0 | 중 | M1-4 | ☐ (VM 준비 시) |
