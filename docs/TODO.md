...
| H-26c | ★ **한글 1byte 첫 타 유실 — 원인 확정 완료 · 처방 단계**([34 §2-8](34-hangul-input-issues.md) 2차 수집 · 사용자 확정 "하나씩") — **2차 수집(08-14 심야)으로 규칙 확정**: *IME 조합 세션 종료 후 첫 1byte keydown 1개가 세션당 1회, 앱 도달 전 소비*(E1~E3 분리 실험 — 평시·연타는 전부 도달, 조합 직후만 1개 증발 · 0.65s 지연 후에도 → 타이밍 아님). 3대 이슈 동반 판정: ① 전환 직후 자모 깨짐 = **앱 방어(leak 조합기)가 복원 실증** ✅ ③ 미완성 조합+Enter = commit 선행 도달로 **정상** ✅(잔여 = 비-Enter 저장 트리거 → ImeGate 규칙 "저장 트리거 전 조합 확정"). 잔여 = ① winit 상류 이슈·버전 조사 ② 없으면 `nbeep-plat` NSEvent 로컬 모니터 보충 주입(mac 한정 · DR-21 — **판정 조건 = 세션당 1회 소비 규칙**) — **수정은 ImeGate(M3-1e) 회귀 박제와 함께 한 번에** | P0 | 중 | M3-1e | 🚧 |
