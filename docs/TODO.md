...
- [ ] **WIME-4 한/영 전환 키** — winit 소스 확인(08-13): 한국어 레이아웃에서 VK_HANGUL → 논리 `NamedKey::HangulMode`·물리 `KeyCode::Lang1`(키보드 드라이버 수준 — IME 비연결에도 도달 예상). 목록 창은 [27 §8](27-typeahead-hangul-composition.md) 앱 토글이 이 키를 소비 · 잔여 = **실기 도달 확인**(토글·상태바 고지) · 대화 창 전환 직후 **첫 키 손실**(mac R2 유형) 재현 여부
