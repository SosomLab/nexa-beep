...
| M4-2 | 파일 **드래그앤드롭** · **폴더 전송**(FR-X-1/2 구조 유지) · **스트리밍**·진행률·취소(FR-X-4/5) · 협상→수락→전송(FR-X-3) · SHA-256 검증(FR-X-6) | P0 | 대 | M4-1 | 🚧 (08-09 — 슬라이스 1·2 + **GUI 연동 완료**: 와이어·수신 조립기(fail-closed) · CLI `/send` · **GUI 드래그앤드롭·⌘Y 수락·진행률** · 수신측 용량 제약(상한=수신자 정책·거절 시 공지) · 액터가 XferInbox 소유. ★청크 64→32KiB(Noise 프레임 상한 실측). 잔여 = **취소 UX**·폴더 전송·재개·드롭 발신 육안 검증) |
| M4-3 | **무해화 게이트 4단계**(FR-S-5~10) — 격리 수신·헤더 봉인·검사 · **OS 격리 표식**(MotW/quarantine, 복원 후 유지 FR-S-8) · **AMSI**(Win, FR-S-15) · 승인 UX 등급별 마찰 | P0 | 대 | M4-1 | 🚧 (08-09 — **승인 UX 완료**: 격리함 화면(등급 칩·신뢰 배지·불일치 경고) · **등급별 마찰**(🔴 2단계 · 승인 경로 키보드 배제) · 승인 = 다운로드 폴더 실체화 + OS 표식(실패도 명시) · 삭제 · 툴바/메뉴 진입. **사용자 실기 확인**. 잔여 = **검사(§6) 어댑터·AMSI**(Win 실기) · 4단계 중 "검사"가 아직 Unavailable 고정) |
...
| ID | 항목 | 상태 |
|---|---|:--:|
| **SEC-1** | **와이어 평문 자동 회귀를 CI에 상시**(FR-S-51) — 이게 있어야 SEC-T1이 사람 기억에 의존하지 않는다 | ☐ **M1-11** |
