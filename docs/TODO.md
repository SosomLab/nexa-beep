...
| M1-7a | **D-8a 상호운용 검증**(08-08 재정의 — **Windows VM 불요, 사용자 확정**) — ① Linux↔Linux: **Docker 컨테이너 N개**(멀티캐스트 도달 검증 완료) ② Windows 단위·회귀: **CI windows-latest**(매 push 자동) ③ Windows 실행·방화벽·IME: **원격 Windows PC**(망 달라도 무관) ④ **맥↔Win LAN 상호운용만 D-8b 실기 날로 이월**(같은 망에 물리는 날 타이밍 실측과 함께). 타이밍 확정은 여전히 불가(가상 손실≈0) | P0 | 중 | M1-4 | ☐ (M1-4 후 Docker로) |
