...
**크리티컬 패스(08-15 현행)** — ~~M0-1 → M0-1b → SP-1(크기 축)~~ ✅ → ~~D-22 확정~~ ✅ → ~~M1-3 와이어 포맷~~ ✅ → ~~M2-7 비동기 펌프~~ ✅ → ~~M4-9 종단 ack~~ ✅ → ~~D-25 → M3-15 설정 영속~~ ✅ → ~~D-18 §3 → M2-5a 신원·핀 영속~~ ✅ → ~~M5-4 배포 2채널~~ ✅(v0.1.4 공개) → ~~M5-1 그룹 개통~~ ✅(08-13 G1~G3) → ~~M3-1e ② IME 상태기계~~ ✅(08-14 · H-25/26/27 완주) → **지금 = 🔴 D-23 확정(M2-4b → M3-8/9/10 연쇄) · 🔴 D-24 범위 확정 · M4-5ⓒ 권한 강등(R-5 종결) · M1 잔여(M1-2·M1-8·M1-9) · M5-1 G4 구성원 편집 · M3-19/14 상태 배지(설계 완료·착수 대기)** → M2-5b 기록 저장(D-18 §4~ 대기) → M5-5 24h 누수 → v1.
