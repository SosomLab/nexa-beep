...
| M4-2 | 파일 **드래그앤드롭** · **폴더 전송**(FR-X-1/2 구조 유지) · **스트리밍**·진행률·취소(FR-X-4/5) · 협상→수락→전송(FR-X-3) · SHA-256 검증(FR-X-6) | P0 | 대 | M4-1 | 🚧 (08-09 — **슬라이스 1·2 완료**: `StreamId::File`+`XferMsg` 6종+`XferInbox`(수락 전 청크=위반·연속 오프셋·즉시 폐기) · CLI `/send`·`/accept`·`/reject` → 무해화 게이트 합류 · **수신측 용량 제약**(상한은 수신자 정책·거절 시 공지) · **1MiB 실기 완주**. ★ 청크 64→32KiB(Noise 프레임 상한 초과 실측). 잔여 = **GUI 드래그앤드롭·진행률 UI·취소 UX**·폴더 전송·재개) |
