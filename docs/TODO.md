...
- [ ] **WIME-1 대화(IME on) 한글 조합 기본** — preedit(밑줄)이 **캐럿 위치**에 단계별로 보이는가(예: "한" = ㅎ→하→한) · Commit 시 본문 삽입 정상 · **조합 중 Backspace = 자소 단위**("한"→"하" — Windows IME 관례, mac과 다름)
...
- [ ] **WIME-3 목록 모드(IME off) 자모 도착 + 타입어헤드 매칭** — `set_ime_allowed(false)` 시 두벌식 키가 **자모 문자로 오는가**([27 §7](27-typeahead-hangul-composition.md) "Windows/Linux 실측 필요" 명시) — 안 오면 목록 타입어헤드 한글 직접 조합이 Windows에서 무동작 · 한글 이름 상대를 두고 **자소 접두 매칭 실동작**까지 확인
- [ ] **WIME-4 한/영 전환 키** — 한영 키(VK_HANGUL)·우Alt가 winit에 어떤 이벤트로 도착하나 · 전환 직후 **첫 키 손실**(mac R2 유형 — [27 §2](27-typeahead-hangul-composition.md)) 재현 여부 · 전환 후 즉시 입력 반영
- [ ] **WIME-5 목록↔대화 전환 IME 토글** — `set_main_ime` 경계에서 조합 세션 잔존("묵은 조합") 여부 · 전환 직후 첫 입력 정상
- [ ] **WIME-6 조합 중 단축키·Enter** — **조합 중 Enter = 조합만 확정**(전송까지 같이 나가면 안 된다 — M3-1d의 핵심 검증점) · 조합 중 **Esc·Ctrl+K 등 단축키**가 조합을 깨거나 오동작하지 않는가(IME 조합 중 단축키 가로채기 금지)
