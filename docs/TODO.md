...
|---|---|---|:--:|:--:|
| **M-1 설계** | 무엇을·어떻게 만들지 확정 | ADR 7종 + 요구/아키텍처 확정 | — | 🚧 **거의 완료** |
...
**크리티컬 패스** — `M0-1 스캐폴딩 → M0-1b 횡단 골격 → SP-1(R-8) → M1 발견 + D-8 스파이크(R-2) → M2 대화·기록 → M3 UI → M4 무해화 → M5 배포`.
...
| [0003](09-adr-0003-transport-abstraction.md) 전송 추상화 | 4계층 경계·InMemory | 📐 **Proposed — 확정 대기** |
...
| D-22 | 🔴 **신원 키 파일 복제 대응 확정**([21 §5](21-identity-spec.md) · R-12) — **U-P1** 발견 패킷 `instance` 난수 16B(같은 키·다른 인스턴스 동시 관측 시 복제 경고) + **U-P2** 상대 `PeerId` == 내 `PeerId`면 세션 거부를 **v1에 넣는가**. ⚠️ **발견 와이어 포맷 확정 전에 답해야 한다** — 이후엔 포맷 변경이다 | P0 | 소 | D-7 | 🔴 **사용자 확정 대기** |
| D-23 | 🔴 **ADR-0010 메시지 등급·알림·수신확인·대체 채널 확정**([24](24-adr-0010-message-priority-notification.md) · DR-25) — ⓐ 등급 3종 + **신뢰 게이트 강등표**([24 §3-3](24-adr-0010-message-priority-notification.md)) 승인 ⓑ **대체 알림을 v1에 넣을지**(R-14 TLS 크기 · SP-2 결과에 종속) ⓒ **N-1~N-4 프로토콜 자리**(등급 1바이트·ack 제어 타입·기록 상태 필드·ActionKind 5종)를 **M2-3/M2-4에 같이 넣는다** — 나중이면 포맷 변경 | P0 | 중 | D-7 | 🔴 **사용자 확정 대기** |

> **설계에 남은 것**: ADR-0003·0004·0005·0006 **사용자 확정** → `Accepted`. **D-22(복제 대응)는 M1-3 와이어 포맷보다 먼저** 답해야 한다. 확정되면 코드 착수.
...
| M1-5 | `core` 피어 목록 — **PeerId 병합**(FR-D-6 · 다중 경로) · 이탈 판정(goodbye+타임아웃 FR-D-8) · 표시 이름 호스트명 기본값(FR-D-9) | P0 | 중 | M1-4 | ☐ |
| M1-6 | 최소 UI — 피어 목록 표시(gfx/ui 최소 경로) · **미검증 배지** | P0 | 중 | M1-5 | ☐ |
| M1-7 | **★ D-8 발견 도달 스파이크 E-1~E-9**([06 §7](06-network-stack.md)) → 타이밍 파라미터 6종 확정 → [ADR-0002 §8](08-adr-0002-discovery-transport.md) 정정. 🔴 **실기 2대+ 필요** | P0 | 대 | M1-4 | ⏸ **사용자 환경 필요** |
...
| M2-2b | 🔴 **안전번호 자릿수 상향**([21 §3-1](21-identity-spec.md) · Q-21-5) — 12자리(≈40비트)는 **생일 공격에 2²⁰**(MITM이 양쪽에 다른 키를 보여주며 쌍을 맞추면 통과). Signal/WhatsApp 60자리 기준으로 상향 + 다이제스트 전체 사용. `sas.rs` 모듈 주석 *"60비트"* → **40비트** 정정 | P0 | 소 | M2-2 | 🔴 **확정 대기** |
...
| M3-10 | **대체 알림 채널**(FR-S-43/44 · R-13) — `Notifier` 포트(DR-21) + **Webhook 어댑터**(P0) · SMTP(P1) · 4중 게이트(검증 상대·부재·미확인·상한) · 묶음 발송 · **L0 봉투 기본** · 자격 증명 암호화 저장([17](17-adr-0005-history-at-rest.md))·로그 마스킹 · 억제 계측 | P1 | 대 | M3-9 · M0-2b | ☐ (SP-2 결과에 종속) |
| M3-11 | **설정 화면**(DR-24 · [14 §10](14-control-ux-architecture.md)) — Entry 레지스트리 · 계층 트리 + AND 토큰 검색(조상 보존·매치 수) · Kind 동적 패널 · 즉시 적용 · `Cmd/Ctrl+,` · 보안 항목(프로필 노출·공유 scope·알림 정책) 확인 훅. ⚠️ 원래 M3-8이었으나 ADR-0010 태스크와 번호 충돌로 재배정(08-08) | P0 | 대 | M3-1 · M3-1b | ☐ |
| M3-8 | **설정 화면**(DR-25 · [14 §10](14-control-ux-architecture.md)) — Entry 레지스트리 · 계층 트리 + AND 토큰 검색(조상 보존·매치 수) · Kind 동적 패널 · 즉시 적용 · `Cmd/Ctrl+,` · 보안 항목(프로필 노출·공유 scope) 확인 훅 | P0 | 대 | M3-1 · M3-1b | ☐ |
...
| M4-7 | **공유 목록 + 가상경로 서버**(FR-X-8/9/11 · [23](23-adr-0009-shared-folders.md)) — 등록·범위 확인 UX · `ShareList`/`ShareGet` · **fail-closed 경로 해석기**(traversal·심링크 안전 회귀 [18] 추가) · 계측 4종 | P1 | 대 | M2-3 · M4-2 | ☐ |
