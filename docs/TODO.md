...
| **M3-2b** | **macOS 메뉴바**(mac PC 몫): `NSStatusItem` 어댑터(objc 직접 — NSSound 선례) — NSImage(컬러 아바타 · 메뉴바 18~22pt@2x) · `button.toolTip` = 이름 · **좌/우클릭 모두 NSMenu**(맥 관례 — 열기·종료) · **AppKit 메인 스레드 강제**(생성·갱신을 winit 메인 루프에서 — 프록시 이벤트) · ⚠ 맥은 "창 닫기 = 앱 유지"가 이미 OS 관례(빨간 버튼) — 스위치 기본값 차등 여부 = 결정지 | P1 | 중 | M3-2 | ✅ (**08-15 mac 세션**: `tray.rs::mac` — NSStatusItem(가변 길이)+NSMenu(이름 헤더·열기·종료 · 좌/우 공통) · 액션 = 선언 클래스 `NbeepTrayTarget` 셀렉터 → 프록시 · 아이콘 = RGBA→NSBitmapImageRep→NSImage 18pt(32px 원본 = 레티나 2x) · **메인 스레드 강제 = MainThreadMarker**(밖이면 None/무시 fail-soft) · 상태는 thread_local(NSStatusItem 비Send) · **실측 = 메뉴바 diff: 35px 아이콘 3개(신원별 아바타) 등장/소거 확인** · **육안 = 메뉴 동작 사용자 확인 ✅(08-15)** · ⏸ 잔여 육안 = close_to_tray on 숨김/복원) |
...
| **M3-2d** | 결정지 2건(🔴 사용자): ① `ui.close_to_tray` **기본값** — 공통 off(예측 가능) vs OS 관례 차등(mac on — 빨간 버튼 관례) ② 숨김 중 수신 시 트레이 아이콘 배지/변형 여부(M3-8 알림과 접점 — 범위 획정) | — | — | M3-2a | ✅ (08-15 사용자 위임 "진행해줘" — 권고안 채택: ① **OS 관례 차등**(mac 기본 on · Win/Linux off — 근거 = **DR-16 "동작 = OS 네이티브"** 기확정 표준: 맥 빨간 버튼 = 창만 닫힘·앱 유지 · `TOGGLE_DEFAULT_OFF` cfg 분기 · 사용자가 바꾼 값은 영속 우선이라 미설정 사용자에게만 적용) ② **숨김 중 수신 배지 = M3-8(알림)로 이관** — 알림 강도는 수신자 판정·신뢰 게이트(🔴 D-23)와 얽혀 단독 선행하면 결정을 앞지른다 · 트레이 v1은 아이콘 변형 없음으로 획정) |
