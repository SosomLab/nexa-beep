...
| **시스템 테마 = 밝기만 추종** | 우리 팔레트가 정본. **`ui.theme` = 시스템(기본)/다크/라이트**(08-29) — 시스템은 OS의 다크/라이트 선호 한 비트만 본다(`nbeep_plat::theme` · Linux 포털 신호 실시간 · Win/mac winit `ThemeChanged`). 맥 시스템 강조색 추종은 **선택 옵션** |
...
| 상태 | 아이콘 | 왜 |
| --- | --- | --- |
| `Unverified` | **`badge-question-mark-nb`** ★ 파생본 | "이 키가 누구 것인지 모른다" — **Lucide 원본은 못 쓴다**([§13-2-1](#13-2-1--물음표는-다시-그렸다-사용자-지적-08-14)) |
...
#### 13-2-1. ★ 물음표는 다시 그렸다 (사용자 지적 08-14)

Lucide `badge-question-mark`의 안쪽 물음표는 **상단이 거의 반원**(`large-arc=0` · 약 178°)이고 꼬리가 짧은 곡선이라 **스템이 없다**. 그 결과 작은 크기에서 훅과 꼬리가 붙어 **동그라미(아크)로 뭉갠다** — 20px 렌더 실측으로 확인했다. 신뢰 배지는 **14px**([§13-3](#13-3-배치--한-행에-표식-셋이-붙는다))라 이 자리에서 "물음표"라는 뜻 자체가 사라진다.

**처방** — `assets/icons-src/badge-question-mark-nb.svg`(**파생본**):

| | 원본 | 파생본 |
| --- | --- | --- |
| 훅 각도 | 약 178°(반원) | **235°** — 왼쪽 아래를 확실히 연다 |
| 꼬리 | 짧은 곡선(중앙에 못 닿음) | 중앙으로 끌어와 **수직 스템** 추가 |
| 점 | y=17 | 그대로(원본과 같은 높이) |
| 바깥 배지 윤곽 | — | **원본 그대로 · 손대지 않음** |

- 획 굵기 2 · 24 격자 · round cap = **Lucide 문법 그대로**라 다른 아이콘과 나란히 놓아도 튀지 않는다.
- ⚠️ **파생본 규약**: 원본 SVG를 고치지 않고 **`-nb` 접미사로 새 파일**을 만든다(`<name>-nb.svg` = nexa-beep 파생). 원본은 그대로 남겨 **출처 추적이 끊기지 않게** 하고, 파일 상단 주석에 **원본 URL · 무엇을 왜 바꿨는지**를 적는다. 이후 다른 아이콘도 작은 크기에서 뭉개면 같은 방식으로 처리한다.
...
```sh
tools/mkicons.sh badge badge-check badge-x badge-alert badge-plus badge-question-mark-nb house globe waypoints
```

굽기 전에 [§13-3](#13-3-배치--한-행에-표식-셋이-붙는다)의 **크기(11/14/12px)에서 실루엣이 살아남는지** 먼저 본다 — **`badge-question-mark`가 실제로 뭉개서 파생본을 만들었다**([§13-2-1](#13-2-1--물음표는-다시-그렸다-사용자-지적-08-14)). 남은 후보는 **`badge-alert`**(느낌표가 가늘다)와 **`waypoints`**(점 4개 + 선 — 12px에서 점이 뭉칠 수 있다)이며, 뭉개면 같은 `-nb` 파생본 규약으로 처리한다.

> ⚠️ **검사 방법**(추정 금지 — 눈으로 본다): `rsvg-convert -w 14 -h 14` 로 실크기 렌더 → 알파를 그대로 읽어 확인한다. 96px에서 멀쩡한 그림이 14px에서 뜻을 잃는 일은 **흔하다**.
