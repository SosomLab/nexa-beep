...
| # | 항목 | 확인 방법 | 기대 |
|---|---|---|---|
| **W-A** | ★ **저장소 마이그레이션**(최우선 — 기존 데이터 지우기 전에) | 기존 `data/`를 가진 채 새 빌드 **첫 실행** | 기존 **핀·그룹 유지**(trust.seg v1→v2 · groups.seg v2→v3 관용) · 이후 재시작에서 ★고정·최근 접속/대화 유지 |
| **W-B** | imgdec 권한 강등 실기(R-5) | 프로필 실사진·수신 이미지 미리보기 | 정상 표시. 이상 시 콘솔 stderr에 `lockdown = windows-mitigation…` 유무가 1차 단서(CI 실측은 통과) |
| **W-C** | 툴바 정렬 드롭다운(IconDropdown) | 4번째 슬롯(호박색 ▼) 클릭 → 4항목 선택 | 팝업 렌더·선택 즉시 재정렬 · **재시작 후 유지** · 설정 List order와 쌍방 동기 |
| **W-D** | 목록 정렬·핀 | 1~2분 관찰 + 우클릭 "목록 상단에 고정" | 그룹 최상단 → 접속 구획 → ①최근 대화 ②최근 접속 · ★ 표시 · **플리커 없음** |
| **W-E** | 2-PC 프로필 전파(M3-21 · Win↔Mac) | Windows에서 이메일 공유 토글 | Mac 상대 카드 **즉시 반영**(역방향도) · 카드에 보더 색·최근 접속/대화 표시 · **비연결 상대 카드 = 조용한 연결**(대화창이 열리면 안 됨) |
| **W-F** | 캐러셀 스크롤 방향 | 프로필 스와치/최근 이미지 가로 스크롤 | Windows 기본 = **현행 방향**(설정 "OS 기본") |
| **W-G** | 그룹 구성원 모달(G4) | 그룹 아이콘/방 헤더 클릭 · 소유자면 행 클릭 | 상태 점 = 목록과 같은 색 · 행 클릭 → 제외 확인 → 상대 화면에서 방 닫힘 |
| **W-H** | 설정 신설 3종 | 설정 열기 | "한글 입력(IME)"(macOS 전용 명기 확인) · Appearance▸목록 보기(정렬·주기·스크롤) · **고급**(백업/복원/초기화 — 피커 경로 처리) |
