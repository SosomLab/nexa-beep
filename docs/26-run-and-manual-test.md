...
### 3-5. 네트워크 점검 모드(netmon · 08-21) — 과도한 송수신 관측·로그 분석

> 설계 = [06 §7-1](06-network-stack.md). **의도적으로 켠 경우에만** 기록한다(기본 off).

1. **켜기**: 설정 ▸ 고급 ▸ **네트워크 점검** ▸ "트래픽 계측 기록" on(주기 기본 10초 —
   같은 섹션에서 변경). 켜는 순간부터의 **델타**만 기록된다.
2. **로그 위치**: `data/logs/netmon-YYYYMMDD.log`(신원 폴더 소속 · 보존/총량은 로그
   설정 공유). 한 줄 = 한 주기: `[시각] netmon dt_ms=10000 dtx=8 drx=16 dself=8
   co=0 cok=0 ci=0 stxf=3 srxf=3 stxb=612 srxb=598`.
3. **읽는 법**: dtx/drx = 발견 발신/수신 패킷 · dself = 내 멀티캐스트 에코(과다 =
   인터페이스 중복 발신 신호) · co/cok/ci = 연결 시도/성립/수락(co≫cok = 재연결 폭풍
   또는 죽은 후보 주소) · stxb/srxb = 세션 바이트(파일 전송 중이면 커지는 게 정상).
4. **과다 경고**: 임계 초과 구간은 줄 끝에 `warn=disco_tx_pps` 같은 태그가 붙고
   **상태바에도 경고**가 뜬다(발견 발신 25pps · 수신 100pps · 연결 분당 30회 초과).
5. **점검 시나리오(권장 주기 점검)**: ① 유휴 10분 — dtx가 광고 주기(0.8s)×경로 수
   수준인지, warn 0인지 ② 상대 강제 종료 후 — co가 백오프대로 잦아드는지 ③ 파일
   전송 중 — stxb만 커지고 발견·연결 축은 조용한지. **회차 결과는 [39 네트워크
   점검 기록부](39-netmon-records.md)에 append**(기준선·판독법 SSOT도 39 §1~2 —
   이상 구간만 로그 원문 첨부).
6. **CLI·자동 실기 수집(NETMON-1 ⓐ · 08-21)**: 어느 모드든(GUI 포함) 환경변수
   `NEXA_NETMON=<파일 경로>`를 주면 설정과 무관하게 그 파일로 주기 기록한다
   (`NEXA_NETMON_SEC` 기본 5초 · `NEXA_IME_TRACE` 선례의 진단 스위치). 예:
   `NEXA_NETMON=/tmp/nm.log NEXA_NETMON_SEC=5 ./nexa-beep.exe --chat-live A --port 47301`.
   **08-21 기준선**(같은 PC 2-인스턴스 · journal/2026-08-21 4차): 발견 대기
   dtx≈3.7pps(자기 에코 = 수신의 50%) · 메시지 1건 ≈ 85B 프레임 1 + ack 62B ·
   유휴 세션 앱 프레임 0 · 64MiB 전송 = 32KiB 청크 그대로(오버헤드 0.25%) ·
   /quit 후 재연결 0.

### 3-6. 공지(브로드캐스트) 빈도 제한·받지 않기 검증 (08-21)

> 발신 = **3초에 1번**(연타는 조용히 버리지 않고 상태바로 안내) · 수신 = 설정으로
> 옵트아웃(그림자 무시 — 발신자에게 드러나지 않음). 와이어 = ChatMessage 상위 니블
> bit2 공지 표식(구버전 수신자는 일반 메시지로 표시 — 전방 호환).

1. **발신 제한**: GUI 메뉴 ▸ 공지 보내기(제목에 "3초에 1번" 명시) → 본문 전송 →
   **즉시 다시** 공지 시도 → 상태바 "공지는 3초에 1번만 보낼 수 있습니다 — …
   (보내지 않았습니다)" + 상대에게 두 번째가 **안 감**을 확인. 3초 뒤 재시도 = 정상 발송.
2. **수신 표시**: 수신측 풍선에 알림(Notice) 등급 링 · CLI 수신은 `[공지] ` 접두.
3. **받지 않기**: 수신측 설정 ▸ 대화 ▸ **"공지(브로드캐스트) 받지 않기"** on(기본 off ·
   저장 즉시 적용) → 발신측에서 공지 → 수신측에 **표시·알림·기록 없음** · 발신측
   풍선에는 전달 점이 안 붙는다(오프라인 수신자와 구분 불가 = 그림자). off로 되돌리면
   다음 공지부터 정상 수신. ⚠ 일반 1:1 메시지·`/notice` 직접 발신은 설정과 무관하게
   **계속 수신**된다(공지 팬아웃만 걸러진다 — 구분 확인이 이 검증의 핵심).
4. **미연결 대기 경로**: 상대 미연결 상태에서 공지 → "즉시 N · 대기 M" → 상대가
   나타나 자동 전달될 때도 `[공지]` 표식·수신측 설정이 그대로 적용되는지 확인.
5. **공지 후 목록 잔류**(08-21 실기 수정 · 2차 보강): 공지 프롬프트를 **Enter로
   제출**한 뒤 메인이 **목록 화면에 그대로** 있는지 — 종전엔 제출 Enter의 잔향/키
   반복이 목록 캐럿 행을 활성화해 그 상대의 대화창으로 들어갔다. 방어 3겹 =
   ① 모달 닫힘 시점 + ② 메인 포커스 복귀 시점의 300ms Enter 가드 ③ 활성화
   드레인에서 모달 열림/가드 중 활성화 최종 폐기(이벤트 순서 무관). Enter를
   **꾹 눌러**(키 반복) 제출해도 목록 유지 확인. 가드가 마우스·다른 키에 영향
   없는지(제출 직후 행 더블클릭 = 정상 열림)도 함께.
6. **공지 창 위치**(08-21): 공지 보내기(그룹 이름 등 프롬프트 공통) 창이 **메인
   (목록) 창 중앙 부근**에 뜨는지 — 종전 OS 기본 위치는 목록과 멀리 떨어졌다.

### 3-7. 릴레이 서버 경유 CLI (08-22 · X-2c) — 서버 기동 → 등록 → 지문 랑데부

> 같은 PC 3프로세스로 전 경로가 검증된다(서버·수신 단말·발신 단말). LAN이 없어도
> 된다 — 서버 랑데부는 **상대 지문(64hex)** 만으로 닿는다(주소 불필요).

```bash
# ① 서버(별도 실행 파일 · 기본 포트 47300 = TCP 제어 + UDP 관측 동번호)
nexa-beepd --port 47300 --verbose        # 기동 출력의 "서버 신원(핀)" = 클라 핀 대상

# ② 수신 단말 — 서버 등록(첫 접속 = 서버 키 자동 핀 · data/server.pin)
nexa-beep --chat-live 이름 --server 127.0.0.1:47300
#   시작 출력의 [신원] 64hex = 내 지문 — 상대에게 알려 줄 값.
#   서버 사용 = 신원 영속 강제(기본 data/identity.key · --identity <파일>로 분리)

# ③ 발신 단말 — 지문으로 랑데부(홀펀칭 → 실패 시 릴레이 자동 사다리)
nexa-beep --chat-connect-via <상대 지문 64hex> --server 127.0.0.1:47300
#   성립 시 "경로: UDP 직결(홀펀칭)" 또는 "릴레이 경유" 1줄 — 이후 대화·파일·확인은
#   기존 그대로(원격 경로면 파일 게이트 곱 판정도 그대로 걸린다).
# chat-live 대기 중에도 /connect <64hex지문> 으로 같은 랑데부가 된다.
```

- ⚠ **같은 PC에서 CLI 둘을 띄울 때는 `--identity <파일>`을 각각 줘야 한다**(09-06 자동화 실측 — 둘 다 exe 옆 `data/identity.key`를 쓰면 같은 지문이라 "상대가 이 서버에 없다"로 끝난다).
- **핀 불일치 = 접속 중단**(시끄럽게 · DR-28) — 서버를 정말 교체했다면
  `data/server.pin`에서 해당 줄을 지우고 재접속(재핀은 사람의 결정).
...
- 실 NAT(공유기 2대) 통과율·Symmetric 폴백은 실기 항목 — 루프백/LAN 검증과
  ...
  (GCP 단발 세션 킷 — musl 정적 바이너리 업로드→상주→철거 · 체크리스트 6항 동봉 ·
  Cloud Run은 TCP/UDP 미지원이라 불가).

**GUI 축(X-2b · 08-22)** — CLI 없이 설정만으로 같은 절차가 된다:

1. **설정 › 서버** — 모드 `managed` · 주소(host 또는 IP) · 포트(기본 47300) 저장 →
   2초 안에 자동 접속(상태바: 첫 접속 = "서버 키를 핀했습니다" / 이후 = "서버 등록 완료").
   실패 = 백오프 재시도(5s→300s 반복) · **핀 불일치 = 경고 모달 + 자동 재시도 정지**
   (핀 줄 삭제 후 서버 설정을 다시 저장하면 재개).
2. **수신** — 자동. 상대가 내 지문으로 걸면 목록·대화에 그대로 뜬다(원격 미등록 키는
   기존 요청 대기 모달 — FR-S-25 동일).
3. **발신** — `⌘/Ctrl+K` 주소 모달에 **상대 지문 64자리**를 넣으면 서버 랑데부
