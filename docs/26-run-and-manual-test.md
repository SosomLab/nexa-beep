...
5. **공지 후 목록 잔류**(08-21 실기 수정): 공지 프롬프트를 **Enter로 제출**한 뒤
   메인이 **목록 화면에 그대로** 있는지 — 종전엔 제출 Enter의 잔향/키 반복이 목록
   캐럿 행을 활성화해 그 상대의 대화창으로 들어갔다(스트레이 Enter 가드 300ms).
   가드가 마우스·다른 키에 영향 없는지(제출 직후 행 더블클릭 = 정상 열림)도 함께.
