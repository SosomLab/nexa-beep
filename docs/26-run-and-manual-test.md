...
| **컨테이너 포트에 연결 거부** — `docker logs` = `Exited (134)` · `시스템 UI 폰트 없음` | 컨테이너에 **`--window`(GUI)** 를 띄웠다 — 헤드리스라 폰트·디스플레이가 없어 즉사, 리슨 자체가 없다(실기 08-13). ⚠️ **무인자도 GUI다**(08-13 기본 변경) — 컨테이너에선 반드시 명시 인자 | 컨테이너는 **`--chat-live --port N`**(터미널 단말 · §4-1). GUI는 호스트에서 |
