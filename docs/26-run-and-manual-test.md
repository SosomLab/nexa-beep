...
|---|---|---|:---:|---|
| `nexa-beep` | 없음 | — | — | 인자 안내 |
| `--window` | GUI | **InMemory**(에코 봇 3명) | — | 오프라인 데모 — **외부와 통신 불가** |
| `--window --live` | GUI | 실물 | — | **표준 실행** |
...
| **컨테이너 포트에 연결 거부** — `docker logs` = `Exited (134)` · `시스템 UI 폰트 없음` | 컨테이너에 **`--window`(GUI)** 를 띄웠다 — 헤드리스라 폰트·디스플레이가 없어 즉사, 리슨 자체가 없다(실기 08-13) | 컨테이너는 **`--chat-live --port N`**(터미널 단말 · §4-1). GUI는 호스트에서 |
...
| Windows 목록 타입어헤드 **한/영 토글·한글 조합**([27 §8](27-typeahead-hangul-composition.md)) | 08-13 구현 — 한/영 키 도달·토글·두벌식 조합의 GUI 실기 미확인(테스트는 매핑·종단 501 green) |
