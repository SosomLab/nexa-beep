...
```
 nexa-beep (bin) ──────────────► 전 크레이트 (조립·어댑터 주입)
 nbeep-imgdec (bin) ── 의존 0 (격리 프로세스 — 신뢰 못 할 입력, R-5)

 net ──► core          crypto ──► core          safe ──► core          store ──► core
 ui  ──► core, gfx      plat  ──► core, gfx      gfx  ── 의존 0(플랫폼 중립 렌더 프리미티브)

              ┌──────────────── core (허브 — nbeep 의존 0) ────────────────┐
              │  도메인 타입 + 포트 트레이트(Transport/Clock/Rng/Meter/…)   │
              └────────────────────────────────────────────────────────────┘
```
