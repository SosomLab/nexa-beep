...
| **TOFU 핀**(FR-S-47) | `PeerId` · 신뢰 등급 · 차단 · 최초 확인/마지막 접촉 · **이름 이력** | **암호화 대상**([17 §4](17-adr-0005-history-at-rest.md) 정정 — 핀 목록 = 인간관계다) · 기록과 **분리된 작은 세그먼트**(부팅 초기 필요) · 원자적 교체 · **못 읽으면 fail-closed**(전부 `Unverified` + "잠김" 표시). 이력은 지우지 않는다. ⚠️ **현재 메모리 전용 — R-17** |
> ★ **신뢰는 두 축이다**(DR-28) — **신원 신뢰는 키**(위 핀)에, **경로 등급은 경로**(아래 경로 이력)에 붙는다. 이동해도 신원은 따라가고 경로만 바뀐다 → [19 §5-1](19-adr-0006-manual-endpoint.md).
...
| **경로 이력**(DR-28) | 노드별 `Locator` 후보 · 마지막 성공 시각 · 연속 실패 · 경로 등급 | ⚠️ **암호화 대상**(FR-S-52 — *"이 사람의 공인 IP 목록"* 은 **위치·소속 이력**이다) · **수명 있음**(FR-D-19 — 실패 상한·다른 키 성립 시 즉시 무효화·보존 상한) |
