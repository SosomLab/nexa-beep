...
> **사용자 확정(08-13)** — 모드 ②는 **동기 저장소(B)**([§3-8](#3-8--확정--컨텐츠-모드는-동기-저장소b-다-사용자-확정-08-13)) · 컨텐츠 서버 파일 흐름은 제안대로 진행([§5-10](#5-10--컨텐츠-서버의-파일--업로드-1회--봉투-n개--능동-pull)).
> **보완(08-13)**: [§3-9](#3-9-기록-등급-l0l2--기본은-안전하게-필요하면-전부) 기록 등급 L0~L2(안전 기본 + 필요 시 전체 기록의 데이터·기술 구조) · [§5-9](#5-9--그룹-파일-어떻게-보내는-게-가장-좋은가-순서대로) 그룹 파일 9단계 설명 · [§9](#9-저장소-구성--한-저장소--배포-분리를-권한다-dr-9-개정-제안) 저장소 구성 권고(DR-9 개정 제안).
...
> ★ **구현된 회전 RID의 실제 동작·수식·사다리 전 과정은 [42 릴레이 랑데부 종단 설명서](42-relay-rendezvous-walkthrough.md)** 에 있다(이 절은 *왜* 그렇게 정했는지, 42는 *어떻게 도는지*).
...
### 5-11. ★ 사용자 확정 흐름(08-20) — 그룹 파일의 모드별 경로 + 보관함 온라인 통합

> 현재 그룹창 파일 전송 = **개인(1:1) 세션 팬아웃**이다. 이 절은 그것을 모드별로
> 어디까지 유지하고 언제 서버 경로로 갈아타는지, 그리고 **보관함(격리함) UX**를 확정한다.
> 구현은 X-3(TODO) — **요청 시 착수**(서버 X-1 이후).

**모드별 경로**

| 모드 | 그룹 파일 경로 |
|---|---|
| Unmanaged ~ **릴레이(①)까지** | **현행 유지** — pairwise 팬아웃(그룹창 전송 = 1:1 세션 N개) |
| **컨텐츠(②) 이상** | **업로드 1회 → 방 종속 보관 → 멤버에게 다운로드 링크**(§5-10의 구체화) |

**컨텐츠 모드 흐름(사용자 확정)**

1. **업로드 = PC → 컨텐츠 서버** — 새 업로드 프로토콜을 만들지 않는다:
   **기존 파일 전송 와이어를 재사용**하고, 서버는 **자동 승인하는 일반 수신 단말**처럼
   받는다(DR-21 이음새 정합 — 클라 쪽 코드는 "상대가 서버"일 뿐이다).
2. ⚠️ **S-3 정합** — "일반 PC처럼 받는다"는 **와이어 형식** 이야기지 평문 수신이 아니다.
   서버로 가는 본문은 **콘텐츠 키로 봉인된 암호문**(P-9 · §5-9 — 열쇠는 멤버별 봉투)이어야
   하고, 서버는 끝까지 못 연다.
3. 서버는 수신물을 **그 그룹 방에 종속**해 보관(§3-8 동기 저장소의 결 · 보존 정책 §3-7)하고,
   업로드가 끝나면 **각 멤버에게 다운로드 가능 통지(링크)** 를 민다.
4. 멤버는 **능동 pull**(§5-10)로 받아 간다 — pull도 **무해화 게이트 동일 통과**(DR-23과 같은 원리).

**★ 보관함(격리함) 온라인 통합(사용자 확정)**

- 보관함 목록에 **로컬 보관물 + 온라인(서버 보관) 항목을 함께** 보인다 — 위치 열 = 로컬/온라인.
- **온라인 항목 클릭 = 다운로드**(무해화 게이트 통과 후 로컬 격리 합류).
- 다운로드 완료 = 위치는 **온라인 그대로** 두되 **"다운로드됨" 상태**로 표시(사본이 로컬 격리에 있음).
- **"저장하기"** = 기존 승인 실체화 경로 그대로(격리 → 실체화 — 온라인 출신도 같은 문을 지난다).
...
| **P-12** | 파일 전송 봉투에 **`blob_id = H(암호문)`(내용 주소) 자리**를 둔다([§5-9](#5-9--그룹-파일-어떻게-보내는-게-가장-좋은가-순서대로)) | 나중에 넣으면 재개·중복 제거·재배포·서버 업로드가 **전부 포맷 변경**이 된다. 지금은 필드 하나다 |
...
## 9. 저장소 구성 — **한 저장소 + 배포 분리**를 권한다 (DR-9 개정 제안)

사용자 질문(08-13): *"서버용 프로그램을 별도 프로젝트로 만들지, 함께 관리하고 배포만 분리할지."*
**DR-9는 현재 "별도 저장소 `nexa-beepd`"** 인데, 설계를 다 적고 보니 **바꾸는 쪽을 권한다.**

### 9-1. 판단 기준은 하나 — **무엇이 제일 잘 깨지는가**

서버 프로젝트에서 실제로 사람을 괴롭히는 건 빌드 시간이나 저장소 개수가 아니라 **와이어 포맷 어긋남**이다.
클라이언트와 서버가 공유하는 것을 세어 보면 분명해진다:

| 공유물 | 어디 |
|---|---|
| 세션 봉투 · 스트림 번호 | [`nbeep-core::mux`](../crates/nbeep-core/src/mux.rs) |
| 파일 전송 메시지(`XferMsg`)·`blob_id` | `nbeep-core` |
| roster·`SGroupMsg`([31](31-adr-0012-shared-group-chat.md)) | `nbeep-core::sgroup` |
| 신원·핀·`PeerId`/`UserId` | `nbeep-core::identity` |
| 저장 세그먼트 포맷([17](17-adr-0005-history-at-rest.md)) | `nbeep-store` |
| 콘텐츠 키 봉투(P-9) | 신설 — **양쪽이 같이 쓴다** |

**서버는 새 프로토콜을 만들지 않는다.** 같은 봉투를 나르고 같은 blob을 보관할 뿐이다.
그런 관계에서 저장소를 가르면 **공유 타입을 복제하거나 크레이트를 퍼블리시**해야 하고,
그 순간 *"클라 0.2.1 ↔ 서버 0.2.0에서만 나는 버그"* 가 시작된다.

> ★ 오늘(08-13)만 해도 **`StreamId::Group`을 추가하면서 `recv_any`가 안 따라온** 일이 있었다.
> **같은 저장소·같은 CI 안에서도** 이런 어긋남이 났다. 저장소가 갈라져 있었으면 몇 주 뒤에나 알았을 것이다.

### 9-2. 권장 — 한 저장소, 워크스페이스 멤버, **배포만 분리**

```text
nexa-beep/
├─ crates/
│  ├─ nbeep-core, nbeep-net, nbeep-crypto, nbeep-store, nbeep-plat …  ← 공유
│  ├─ nexa-beep            (클라이언트 GUI/CLI)
│  ├─ nbeep-imgdec         (격리 디코드 자식)
│  └─ nexa-beepd           ← ★ 서버 (새 멤버)
└─ .github/workflows/
   ├─ ci.yml               (전체 — 어긋남을 여기서 잡는다)
   ├─ release.yml          (클라이언트 5타깃 × 2채널)
   └─ release-server.yml   ← ★ 서버 컨테이너 (별도 파이프라인·별도 태그)
```

| 얻는 것 | 내용 |
|---|---|
| **어긋남이 컴파일 에러로** | 공유 타입을 고치면 서버도 같은 PR에서 깨진다. 가장 값싼 발견 시점 |
| 교차 버전 테스트 | 한 CI에서 *클라 vN ↔ 서버 vM* 시나리오를 돌릴 수 있다 |
| 1인 개발 비용 | 이슈·PR·릴리스·문서를 두 곳에서 굴리지 않는다 |
| 설계 문서 근접 | 이 문서(32)와 코드가 같은 저장소에 있다 |

### 9-3. ⚠️ 대신 **반드시 지킬 가드레일 4개**

한 저장소의 위험은 **서버가 클라이언트를 오염시키는 것**이다. 그래서 규칙을 박는다.

| # | 가드레일 | 왜 |
|:--:|---|---|
| **W-1** | **`nexa-beepd`는 클라이언트의 의존 그래프에 절대 들어가지 않는다** — 방향은 항상 `beepd → core`, 역참조 금지 | 들어가는 순간 클라이언트 산출물에 서버 의존이 딸려 온다 |
| **W-2** | **예산 게이트(DR-5)는 클라이언트만 잰다** — `cargo build -p nexa-beep` 기준(지금 CI 그대로) | 서버 의존(DB·TLS)이 **≤10MB/타깃·런타임 의존 0**을 오염시키면 안 된다 |
| **W-3** | **서버 전용 의존은 `nexa-beepd`에만.** 공유 크레이트에 서버 편의를 넣지 않는다 | `nbeep-core`가 무거워지면 클라이언트가 같이 무거워진다 |
| **W-4** | **릴리스 태그를 분리** — 클라 `v0.x.y` / 서버 `beepd-v0.x.y` · 자산·체인지로그 별도 | 배포 주기가 다르다(클라 = 데스크톱 5타깃, 서버 = 컨테이너) |

**DR-12(퍼미시브 온리)·DR-7·DR-13은 서버에도 그대로 적용된다** — 같은 저장소라 규약이 자동으로 따라온다는 것도 이점이다.

### 9-4. 언제 갈라야 하나 (되돌릴 조건)

지금 합치더라도 **나중에 가르는 건 쉽다**(크레이트 하나를 옮기면 된다). 반대는 어렵다.
아래 중 하나가 실제로 생기면 그때 분리한다:

| 신호 | 왜 그때는 갈라야 하나 |
|---|---|
| 서버에 **외부 기여자·공개 이슈**를 받기 시작 | 클라이언트 저장소의 공개 표면이 달라진다 |
| 서버가 **다른 라이선스·다른 팀** 소유가 됨 | 소유권이 갈리면 저장소도 갈린다 |
| 워크스페이스 빌드가 **개발을 방해할 만큼** 느려짐 | 실측으로 판단(체감 전엔 추측하지 않는다) |
| 서버가 **GPL/무거운 런타임**을 요구 | DR-12·DR-5와 충돌 — 격리가 필요 |

> ### 결론
> **지금은 한 저장소가 맞다.** 가장 잘 깨지는 것이 *와이어 어긋남*이고, 한 저장소는 그걸
> **컴파일 시점에** 잡아 준다. 저장소를 나눠서 얻는 것(공개 표면 분리·빌드 격리)은
> **아직 필요하지 않은 문제**다. → **DR-9 개정 제안 = Q-32-13.**
...
| 모드 ② | **ADR-0007 다중 기기 구현**(★ B 확정으로 **필수 확정** — §3-8) + [17](17-adr-0005-history-at-rest.md) 저장 확정(M2-5b) | §3-3 — 순서를 뒤집으면 서버가 신원 권위가 된다 |
...
| ~~Q-32-7~~ | ~~P-9를 v1.5 그룹 와이어에 지금 넣을 것인가~~ | ✅ **확정(08-21) = 자리 예약 + 문서 확정** — 그룹 와이어가 **kind 1B 프레이밍**이라 새 kind 추가 = 와이어 변경 없음(구버전 = 미지 kind 조용히 폐기 · 전방 호환)임을 확인하고, **P-9 전용 kind 8을 예약**(`sgroup.rs` `K_CMSG_RESERVED` — `[uid‖seq‖blob_id(P-12)‖키 봉투‖암호문]` 형태 명문화). 실구현·검증은 소비자가 생기는 서버 축(X-1/X-3) 착수 때 — 검증 수단 없는 사변적 필드를 지금 넣지 않는다 |
...
| ~~Q-32-10~~ | ~~모드 ②는 A인가 B인가~~ | ✅ **확정(08-13) = B 동기 저장소**([§3-8](#3-8--확정--컨텐츠-모드는-동기-저장소b-다-사용자-확정-08-13)) — ADR-0007이 선행 조건이 된다 |
| **Q-32-11** | 송수신 기록(누가·누구에게·언제) **보존 기간 기본값**([§3-9](#3-9-기록-등급-l0l2--기본은-안전하게-필요하면-전부)) | 무기한은 없다. 90일? 조직 설정 허용 범위는? |
| ~~Q-32-13~~ | ~~DR-9 개정 — 서버를 같은 저장소에?~~ | ✅ **확정(08-21)** — 권고안 그대로 **한 저장소 + 배포 분리**(사용자 지시 "별도 실행 파일·별도 배포"가 §9와 일치). 실물 = `crates/nexa-beepd` 워크스페이스 멤버 + `beepd-v*` 태그 + release-server.yml · W-1~W-4 준수 → [10 DR-9 개정](10-decision-record.md) |
| **Q-32-14** | 기록 등급 기본값과 **조직이 올릴 수 있는 상한**([§3-9](#3-9-기록-등급-l0l2--기본은-안전하게-필요하면-전부)) | L0 기본은 정했다. **L2를 제품이 허용할 것인가**(Q-32-4와 한 쌍) · 보존 상한은 얼마인가 |
| ~~Q-32-15~~ | ~~UDP 세션 전송 도입?~~ | ✅ **확정(08-20 선언 → 08-21 착수·구현)** — 도입한다. 방식 = X-UDP-a ⓐ 자작 ARQ(`nbeep-net::arq`+`UdpLink` — 08-21 구현 · 결정적 주입 회귀 10종). 서버 MVP(X-1)도 같은 날 개통 — [TODO X-UDP](TODO.md) |
| **Q-32-12** | **구성원 재배포(swarm)를 v1.5 그룹에 넣을 것인가**([§5-9](#5-9--그룹-파일-어떻게-보내는-게-가장-좋은가-순서대로)) | 서버 없이 N배 업링크를 크게 줄인다. 다만 P-12(내용 주소)가 선행 |
...
## 12. 08-17 검토 — Unmanaged/Managed · 홀펀칭 실현성 · 타입별 경계 · 릴레이 MVP

> **맥락**: 클라이언트에 **서버 설정 영역**을 추가(기본 Unmanaged · 값만 저장 ·
> 네트워킹은 TODO). 이 절은 그 위에 얹힐 네트워킹의 **설계 검토**다 — 구현
> 지시가 아니다(서버 작업은 명시 전까지 개발 제외 · 사용자 확정 08-17).

### 12-1. 클라이언트 모드 축 — Unmanaged / Managed

| 축 | 값 | 뜻 |
|---|---|---|
| `net.server.mode` | **`unmanaged`(기본)** | 서버 없음 = 순수 LAN(현행 · S-0). |
| | `managed` | **서버 1개 등록 필수**(IP/도메인 + 포트) · 그 서버를 경유. |
| `net.server.type` | **`auto`(기본)** | 핸드셰이크 때 **서버가 제공하는 타입**으로 자동 설정. |
| | `relay`·`content`·`registered` | **Managed에서만 직접 선택**(서버가 그 타입을 지원할 때만 성립). |

- 등록 절차(Managed 선택 시) = 주소·포트 입력 → **서버 공개키 TOFU 핀**([§2-4]) →
  핸드셰이크에서 서버가 자기 타입·정책을 통지 → 클라가 auto면 그대로 채택.
- **타입은 서버가 소유한다** — 클라가 `content`를 골라도 서버가 gateway면 gateway로
  성립(서버가 못 하는 걸 클라가 강요할 수 없다). auto가 기본인 이유.

### 12-1b. 서버 타입 3종 — 누적 정의 (사용자 확정 08-17)

**Gateway를 별도 타입으로 두지 않는다** — 홀펀칭(랑데부)은 릴레이의 기본
동작이라 릴레이에 포함된다. 세 타입은 **기능 누적**이다:

| 타입 | = | 서버가 제공 |
|---|---|---|
| **Relay** | 홀펀칭 | 서버 경유 홀펀칭 역할만 · **별도 가입 없이 핸드셰이크만으로 서버 등록** · 같은 서버 사용자와 직접 세션(폴백 = 서버 중계) |
| **Content** | Relay + 파일 | 위 + **파일 송수신 서버 경유**: 발신이 서버에 올리면 수신자가 서버에서 다운로드 · 설정에 따라 **자동 다운로드 or 직접 다운로드** · 서버는 여전히 **암호문만 보관**(봉투 원리 · S-3) |
| **Registered** | Content + 가입 | Content의 모든 기능 + **사전 Signup·승인 통과자만 서버 등록 가능**(웹 가입 등 외부 경로) · 비회원 상호 불가시성([§12-5]) |

- **auto(기본)** = 핸드셰이크 때 서버가 자기 타입을 통지 → 그대로 채택.
- 이전 초안의 "Gateway 단독(릴레이 폴백 없음)"은 폐기 — 릴레이가 펀치+폴백을
  함께 가지므로 별도 타입이 아니다.

### 12-2. ★ 홀펀칭 실현성 (사용자 우선 질문) — **가능하되 선행 하나가 관문**

**질문**: 각 단말을 터널링 노드로 두고 서버 경유로 홀펀칭을 구현할 수 있나?
**답**: **표준 기법(STUN 랑데부 + 동시 열기 + 실패 시 TURN 릴레이 폴백)으로
가능하다.** 이 사다리가 ADR-0013의 **Gateway(=STUN/펀치) → Relay(=TURN 폴백)**
와 정확히 겹친다. 다만 **UDP 세션 전송이 선행**돼야 한다.

**동작(서버 = 시그널링만, 종단은 직접 E2E)**:
1. 두 클라가 서버에 붙어 **서버가 밖에서 본 각자의 공인 (IP:포트)** 를 얻는다(STUN).
2. 서버가 서로에게 상대의 관측 엔드포인트를 알린다(시그널링 — **엔드포인트 = 메타**).
3. 양쪽이 **동시에** 그 주소로 UDP를 쏜다 → 각 NAT에 매핑이 열려 **직접 경로** 성립.
4. 그 위에 **Noise_XX 종단 핸드셰이크**(서버는 봉투도 못 본다 · S-3).

**도달 범위(정직하게)**:

| NAT 유형 | 홀펀칭 | 비고 |
|---|---|---|
| Full-cone·Restricted·Port-restricted | ✅ | 가정·사무실 공유기 **대다수** |
| 한쪽만 Symmetric | ⚠️ 부분 | 포트 예측 보조로 일부 성공 |
| 양쪽 Symmetric | ❌ | **TURN 릴레이 폴백 필수**(서버가 암호문만 나름) |
| CGNAT(통신사 다단 NAT) | ⚠️~❌ | 릴레이 폴백으로 흡수 |

→ **"펀치 성공 = 직접, 실패 = 릴레이"** 는 S-1(LAN 우선) 아래 3번째 경로다([§0-경로 사다리]).

**⚠️ 선행 관문 — UDP 세션 전송**: 현재 세션 전송은 **TCP**(`TcpLink` · Noise over
TCP)다. 홀펀칭은 **UDP가 사실상 필수**(TCP 동시 열기는 NAT 지원이 들쭉날쭉·불안정).
따라서 홀펀칭 이전에 **신뢰성 있는 UDP 세션 계층**(재전송·순서·혼잡 — QUIC류 또는
경량 자작)이 필요하다. 이게 **가장 큰 선행 작업**이고, 없으면 펀치로 뚫은 UDP
경로를 mux·파일 전송이 못 쓴다. 발견용 UDP(`udp.rs`)는 있으나 세션용이 아니다.

**"터널링 노드" 해석 보정**: 노드마다 L3 VPN 터널을 세우는 게 아니라, **세션마다
펀치된 UDP 경로에 기존 Noise+mux를 얹으면** 충분하다(터널 오버헤드 불필요 · 예산
DR-5 정합). 서버는 랑데부·시그널링·(폴백 시)릴레이만 — 터널의 당사자가 아니다.

**예산·원칙 정합**: T0 무권한 ✅(홀펀칭은 관리자 불요) · S-3 ✅(엔드포인트만·릴레이도
암호문) · DR-5 ⚠️(풀 ICE 스택(libwebrtc)은 금지 — **경량 STUN+펀치+TURN 자작**이라야
예산 안).

### 12-3. Gateway 타입 — 어디까지 연결되나

**정의**: 서버의 유일한 일 = **랑데부 + 홀펀칭 코디네이션**. 저장 안 함·계정 없음·
릴레이 안 함(순수). 성립하면 서버는 경로에서 빠진다.
- **할 수 있는 것**: 프레즌스(이 서버에 누가 온라인인가) · 엔드포인트 교환 · 동시 열기 신호.
- **경계**: 서버는 **누가·누구와·언제 + IP 엔드포인트**(메타)만 안다. **내용은 못 본다**(E2E).
- **한계**: 양쪽 Symmetric NAT는 **Gateway만으론 못 뚫는다** → 여기서 멈추면 그 쌍은
  연결 불가. 그래서 **Relay 폴백이 없는 Gateway 단독은 "대다수 연결, 소수 실패"** 다.

### 12-4. Content 타입 — 필요한 기능과 경계선

**정의**(§0 모드 ②): 서버가 **읽을 수 없는 암호문 덩어리를 보관·전달**한다.
- **필요 기능**(LAN이 못 하는 것): ① **오프라인 큐**(수신자가 오프라인이어도 봉인
  봉투를 맡아 뒀다 깨어나면 전달 — LAN은 양쪽 동시 온라인 필수) ② **그룹 팬아웃
  효율**(§5-10 — 1회 업로드 + N개 키 봉투) ③ 수신자 릴레이/알림 봉투(ADR-0010).
- **경계선(넘으면 다른 제품)**: 서버는 **봉투 메타(발신·수신·크기·시각)까지만**,
  **평문·파일 이름·검색어는 절대 못 본다**(전부 암호문 · S-3). "서버가 내용을 읽어
  정책·모더레이션·필터"를 하는 순간 이 제품이 아니다 — 그 선을 코드가 아니라
  **와이어가 강제**한다(서버에 평문이 도달할 경로 자체가 없음).

### 12-5. 채팅 대상 범위 — 타입별 (사용자 확정 08-17)

| 타입 | 채팅 대상 | LAN 사용자 | 나의 노출 |
|---|---|---|---|
| Unmanaged | LAN만 | ✅ 보임 | LAN에 방송 |
| Gateway·Relay·Content | **LAN ∪ 같은 서버 사용자** | ✅ **여전히 보임**(서버는 증강) | LAN + 서버 |
| **Registered** | **서버 등록 사용자만** | ❌ **숨김** | ❌ **아무에게도 안 보임** |

- **Relay·Content = 증강(union)**: 서버가 LAN 목록에 **더한다**(원격·다른 망 사용자
  추가). LAN 발견은 그대로 산다 — 사용자 확정.
- **Registered = 대체(replace) + 폐쇄**: 서버 명부가 **LAN 발견을 대신**한다.
  - **사전 가입·승인 통과자만** 서버에 붙는다(웹 가입 등 외부 경로 — 핸드셰이크만으론
    가입 불가 · 이게 다른 타입과의 결정적 차이).
  - **상호 불가시성**: 나는 비회원에게 **안 보이고**, 나도 비회원을 **안 본다**.
    → Registered 모드에선 **LAN 브로드캐스트를 억제**(발견 송신·수신 모두 끔)하고
    **서버 명부만** 목록의 원천으로 삼는다. 이게 "당연히 나도 표출되지 않아야"의 구현.
  - 다른 타입(핸드셰이크=가입)과 달리 **명부가 게이트** — 서버가 인증한 사용자만
    서로를 본다. 조직 폐쇄망 모델(§4 모드 ③ 관리와 연속).

### 12-6. 가장 간단한 릴레이 서버 — MVP 명세

> ★ **구현됨(08-21 · 사용자 명시 요청 — X-1 ✅ MVP)**: `crates/nexa-beepd`(별도 bin) +
> `crates/nbeep-relay`(공유 와이어·클라 어댑터) + `nbeep-net::{arq,udplink}`(X-UDP-a/b).
> 아래 기능 1~4 전부 + 루프백 홀펀칭 배관까지. 잔여 = 실 NAT 실기·경로 사다리(X-UDP-e)·
> GUI/CLI Managed 배선(X-2b/2c). 아래 명세는 원문 유지(구현 대조 기준).

**목표**: Gateway(랑데부+펀치) + **Relay 폴백**(펀치 실패 시 암호문 중계)까지.
Content·Registered는 MVP 밖(각각 오프라인 저장·명부/인증이 큰 축).

**기능(MVP)**:
1. **프레즌스 등록** — 클라가 붙어 `{RID, 관측 엔드포인트}` 등록(§2-3 — `PeerId`
   원본 금지 · **회전 RID**로 R-18 확대 방지).
2. **랑데부/시그널링** — "RID_A가 RID_B에 닿고 싶다" → 서버가 양쪽에 상대
   엔드포인트 전달 + 동시 열기 타이밍.
3. **Relay 폴백** — 펀치 실패 시 서버가 **양방향 이미 성립한 세션의 암호문만**
   중계(단발 요청 응답 없음 — 증폭 방지 §2-6 · RID 토큰 버킷).
4. **서버 자기 핀** — 클라가 서버 공개키 TOFU 핀(§2-4 — 릴레이 MITM 방지).

**명시적 비목표(MVP)**: 계정·명부(→Registered) · 오프라인 저장(→Content) · 그룹
팬아웃 · 관리 콘솔 · 웹 가입.

**선행 의존(클라 쪽)**: **UDP 세션 전송**(§12-2 관문) — 이게 없으면 릴레이 서버만
있어도 홀펀칭이 무의미. 즉 릴레이 서버 착수 = UDP 세션 계층 착수와 한 묶음.

**개발 규모(러프)**:

| 파트 | 범위 | 러프 |
|---|---|---|
| UDP 세션 전송(클라·선행) | 재전송·순서·혼잡 경량 계층 or QUIC류 검토 | **대**(수 주) |
| STUN/펀치 코디네이션 | 서버 시그널링 + 클라 ICE-lite 에이전트 | 중 |
| Relay 폴백 | 서버 blind 중계 + RID 토큰 버킷 | 소~중 |
| 서버 골격 | async TCP/UDP·프레즌스 맵·서버 신원 키 | 중 |
| **합계** | Gateway+Relay MVP(Content/Registered 제외) | **가장 큰 건 UDP 전송 선행** |

> **결론**: 릴레이 서버 자체는 작지만, **UDP 세션 전송이라는 큰 선행**이 관문이다.
> 이 선행 없이 서버만 만들면 못 쓴다 — 그래서 "서버부터"가 아니라 **UDP 전송을
> 먼저 결정**하는 게 순서다(D-29 하위 신규 질문 Q-32-15).

### 12-7. 검토(08-22) — 같은 서버 사용자 **자동 목록 표시**는 가능한가

> 실기 발단: 맥·윈도우가 같은 서버에 등록됐는데 서로 목록에 안 뜬다(현행 = 설계
> 그대로 — 첫 연결은 지문). 사용자 질문 = "자동으로 목록에 표시되도록 구현 가능한가".

**결론 = 가능하고, 이미 확정된 방향과도 맞는다 — 단 공개는 옵트인 축으로.**

- **찬성 근거(이미 확정된 설계)**: [§12-5](#12-5-채팅-대상-범위--타입별-사용자-확정-08-17)가
  Relay/Content의 채팅 대상을 **"LAN ∪ 같은 서버 사용자"**(서버가 목록을 **더한다**)로
  사용자 확정했고, [§12-3]도 "프레즌스(이 서버에 누가 온라인인가)"를 서버의 일로
  명시한다. 자동 표시는 이 확정의 실물화다.
- **긴장(지켜야 할 원칙)**: [§2-3]의 회전 RID는 서버가 신원 기록을 **일부러 못
  남기게** 한 장치다. 사실 관계를 정직하게: 제어 세션이 Noise_XX 상호 인증이라
  **서버는 핸드셰이크 순간 클라 공개키를 본다**(익명성 주장은 애초에 없다) —
  다만 현행 서버는 그것을 **어디에도 담지 않는다**(프레즌스 맵 = RID 키). roster는
  "공개키를 담고 배포"하는 행위라, **본인이 켠 것만** 담는 옵트인이어야 §2-3의
  정신(비기록 기본)이 산다. DR-22(옵트인 노출)와 같은 결.

**권고안 — Announce 옵트인 프레즌스(roster)**:

| 축 | 내용 |
|---|---|
| 와이어 | `C2s::Announce{listed}` + `S2c::PeerUp{peer}`/`PeerDown{peer}`(입장 시 스냅숏 + 델타). 미지 kind 무시 = 구버전 서버·클라 전방 호환(구서버 = 자동 표시만 없음) |
| 서버 | 켠 연결만 `PeerId` 보관·연결 종료 시 즉시 제거(**메모리만** — 저장 0 불변) · 배포 대상 = 같은 서버의 Announce 사용자 |
| 클라 | 설정 `net.server.announce`(서버에 내 존재 공개) · 수신한 peer = 목록에 **서버 상대 행**(지문 유도 이름 `beep-xxxx` · 경로 = 서버) · 클릭 = 기존 `connect_via` 사다리 → 성립 후 프로필 프리페치가 이름·아바타를 채운다(기존 경로 무변경) |
| 노출 범위 | **존재(공개키)만** — 이름·프로필은 여전히 연결 후 P2P(DR-22 불변). LAN 발견은 이미 존재+이름을 방송하므로 이것은 LAN보다도 좁은 노출 |
| 스팸 | roster는 **수신이 아니다** — 낯선 이의 접속 시도는 roster 없이도(지문만 알면) 가능했고, 오면 기존 게이트(원격 미등록 = 요청 대기 모달 · 미검증 무음 · 파일 차단)가 그대로 막는다. 강화는 [38 차단 4축](38-receive-restriction-design.md)(BL) 몫 |
| 공수 | 서버 소 · 와이어 소 · 클라 중(목록 통합·설정·i18n) + **beepd 재배포**(`beepd-v0.2.5` — musl 자산 교체 수 초) |

**🔴 Q-32-16(사용자 확정 필요) — `net.server.announce` 기본값**:
ⓐ **기본 on(권고)** — §12-5 확정("Managed 선택 = 서버 노출 의사")과 정합 · 사용자가
원한 "자동 표시" UX가 기본으로 성립 · 노출은 존재뿐(LAN 방송보다 좁다) /
ⓑ 기본 off — DR-22 문자 그대로(공개 기본 서버에서 낯선 이에게 존재 노출 회피) ·
단 양쪽 다 켜야 보여서 "자동"이 실질 무효가 된다. X-2d(핀 상대 프레즌스 조회)는
roster가 Announce 사용자를 덮으므로 **비공개 상대 한정 보완**으로 축소.
...
### 13-1. 원칙 — CLI 전용 네트워킹은 만들지 않는다

| # | 원칙 | 근거 |
|---|---|---|
| C-1 | **서버 경로 = `Link` 형제 구현 하나**(`RelayLink`) — CLI·GUI가 같은 어댑터를 쓴다 | DR-21(이음새) · X-UDP-b와 같은 자리. 위층 Noise·TOFU·mux·채팅·파일 **무변경** |
| C-2 | **신원 영속이 선행** — 매 실행 `Identity::generate()`인 지금의 CLI로는 Managed가 무의미하다(핀·attestation·수신확인 게이트 전부 키에 묶인다) | §4-2(공개키 등록) · N-2 프라이버시 게이트 |
| C-3 | 서버 파라미터의 SSOT = **기존 설정 키**(`net.server.*`) — CLI 인자는 **그 위의 세션 한정 오버라이드** | DR-27(설정 단일 원천) · GUI와 값이 갈리면 두 벌이 된다 |
| C-4 | S-0~S-3 그대로 — CLI도 서버 없이 완전하고(LAN 직접), 서버는 봉투만 본다 | §1 불변식 |

### 13-2. 실행 형태 (제안 — 이름은 착수 시 확정)

```
nexa-beep --chat-live 이름 --server <host[:port]>       # 서버 프레즌스 등록 + LAN 광고 병행
nexa-beep --chat-connect-via <상대지문|RID>             # 서버 랑데부로 연결(주소 불필요)
nexa-beep --enroll <초대코드> [--server <addr>]         # 모드 ③ 가입(1회 · §4-2)
nexa-beep --identity <파일>                              # 신원 영속(선행 C-2 · 기본 = data/identity.key)
```

- `--server` 생략 = `settings.cfg`의 `net.server.address:port`(Managed일 때) — C-3.
- `--identity` 는 **서버와 무관하게 먼저 넣을 수 있다**(GUI keyfile 로직 재사용) —
  넣는 즉시 CLI에도 핀 지속·수신확인(N-2)·attestation 자리가 생긴다.

### 13-3. 접속 흐름 — §12 MVP 위에 그대로 얹는다

1. **서버 핀**(§2-4): 첫 접속에서 서버 공개키 TOFU 핀(신원 폴더 `server.pin` — trust.seg 문법 재사용). 바뀌면 시끄럽게.
2. **프레즌스 등록**(§12-6 기능 1): 회전 RID 등록 — `PeerId` 원본 금지(§2-3 · R-18).
3. **상대 지정**: GUI에서 복사한 지문 또는 RID. 서버는 랑데부만(§12-6 기능 2).
4. **경로 사다리**(S-1 · X-UDP-e): LAN 직접 → 홀펀칭(UDP — X-UDP 선행) → **Relay 폴백**. ★ **1차는 TCP Relay 폴백만으로 성립** — UDP(Q-32-15) 확정 전에도 "서버 경유 CLI 대화"는 가능하다(느릴 뿐).
5. 성립한 링크 위는 **기존 그대로** — Noise 핸드셰이크 → TOFU → `run_interactive`. 경로 등급만 `Relay`(DR-28 — 신원 신뢰 불변).
6. **모드 ③(Registered)**: `--enroll` = 초대코드 + 공개키 제출 → attestation 수신 + **조직 루트 키 핀**(§4-2). CLI 표시 = `[조직] 확인됨 — SAS 대체 아님`(§4-1). Registered 대상 규칙(서버 명부만·상호 불가시성)은 §12-5를 따른다.

### 13-4. 구현 단계 (차후 · 의존 순서)

| 단계 | 내용 | 의존 | 비고 |
|---|---|---|---|
| **P0** | CLI 신원 영속(`--identity` · keyfile 재사용) | 없음 — **지금도 가능** | 수신확인(N-2)·핀 지속의 선행이기도 하다 |
| **P1** | `RelayLink`(TCP) + 서버 핀 + 프레즌스/랑데부 + `--chat-connect-via` | X-1(서버 MVP) | 홀펀칭 없이 성립(13-3 ④) |
| **P2** | UDP 홀펀칭 편입 | X-UDP-a~e | 경로 사다리 완성 |
| **P3** | `--enroll`(모드 ③ attestation·조직 루트 핀) | X-1 Registered 축 | §4-2 흐름 그대로 |
