...
### 3-8. ★ 갈림길 — **"도착한 단말에서만"** vs **"다른 PC에서도"**

사용자 보완(08-13)에 **서로 다른 두 모델**이 섞여 있다. 하나를 고르지 않으면 설계가 흔들린다.

| | **A · 저장형 중계**(store-and-forward) | **B · 동기 저장소**(sync store) |
|---|---|---|
| 근거 문장 | *"해당 단말에 도착했을 경우만 해당 단말에서 볼 수 있다"* | *"다른 PC에서 접속해도 사용"*(모드 ② 최초 정의) |
| 내용을 누구 키로 잠그나 | **수신 단말 키**(`PeerId`) | **사용자 마스터 키**(기기별 래핑) |
| 새 PC가 과거를 여나 | **❌ 못 연다**(설계다) | ✅ 연다 |
| 다중 기기(ADR-0007) | **불필요** | **선행 필수**([§3-3](#3-3-c-2-해법--다중-기기adr-0007가-선행-조건이다)) |
| 서버 역할 | 오프라인 큐 + 배달 증명 | 암호문 저장소 |
| 복잡도 | 낮음 — **지금 만들 수 있다** | 높음 |
| 키 분실 | 그 단말 것만 잃음 | 복구 시드로 전체 복구 |

> ★ **A는 모드 ②의 축소가 아니라 다른 제품이다.** A만 하면 "서버에 보관"의 실체는
> **"상대가 꺼져 있어도 도착한다"** 이지 *"어디서든 이어 쓴다"* 가 아니다.
> 둘 다 정당한 제품이지만 **한 문장으로 뭉치면 안 된다.**

**권장 — A로 시작하되 B로 갈 수 있는 형태로 담는다.** 방법은 이미 [§5-1](#5-1--구조적-문제--pairwise-팬아웃--릴레이--n배-업링크)에 있다:

```text
본문:  Enc(K_content, plaintext)            ← 한 번만 암호화. 이게 blob.
봉투:  Wrap(K_recipient_device, K_content)  ← A. 지금 만든다
       Wrap(K_user_master,      K_content)  ← B. 나중에 봉투 하나만 더 붙인다
```

**본문 재암호화가 없다.** 봉투를 하나 더 붙이면 A→B다. [20 V1-4](20-adr-0007-multi-device-identity.md)의
"한 겹 래핑" 제약이 정확히 이걸 위해 있었고, 이제 **그룹 팬아웃(§5-1)과 같은 메커니즘**이 됐다.

> ### ★ 하나의 구조가 세 문제를 동시에 푼다
> **콘텐츠 키 + 수신자별 봉투** = ① 그룹 N배 업링크 해소 ② A→B 이행 ③ 서버 보관.
> 이게 이 문서에서 가장 값이 큰 발견이다 → [§8 P-9](#8-v1이-지금-지켜야-할-것--선행-제약)가 그래서 최우선이다.

### 3-9. "언제 누가 누구에게"를 남긴다 — **가능하다. 대가를 적는다**

사용자 요구: 서버는 평문을 안 갖되 **송수신 기록(언제·누가·누구에게)** 과 암호문은 보관한다.
**기술적으로 전부 가능하다.** 다만 그 기록이 정확히 [§5-4](#5-4--새로-생기는-위험--서버는-그룹-그래프를-본다)가 경고한 **사회 그래프**다.

| 항목 | 판정 |
|---|---|
| 평문 미보관 | ✅ 가능 — 서버는 blob과 봉투만 |
| 원 수신자만 열람 | ✅ 가능 — 봉투가 수신자 키로만 열린다 |
| 배달 증명(언제 도착) | ✅ 가능 — 서버가 pull 시각을 기록 |
| **"누가 누구에게" 기록** | ✅ 가능 — 하지만 **이건 내용 못지않게 민감하다** |

> ⚠️ **정직하게** — 메타데이터는 종종 내용보다 많이 말한다. *"A가 매일 밤 B에게 보낸다"* 는
> 내용을 몰라도 충분히 드러낸다. 이걸 **모르고** 남기는 것과 **알고** 남기는 것은 다르다.

**그래서 남기더라도 이렇게 남긴다**

| 규칙 | 내용 |
|---|---|
| **보존 기간 상한** | 무기한 금지. 기본 90일 등 — 조직/사용자 설정 · 만료 자동 삭제 |
| **가명 유지** | 기록의 주체는 계정이 아니라 **안정 가명 ID**(모드 ③에서만 실명과 연결) |
| **열람 주체 명시** | 누가 이 기록을 볼 수 있나(본인 · 조직 관리자 · 운영자)를 문서와 UI에 |
| **집계만 노출** | 관리 화면 기본은 통계. 개별 왕래 조회는 **감사 기록을 남기는 별도 권한** |
| **사용자 고지** | 모드 ② 이상에서는 "송수신 기록이 서버에 남습니다"를 상시 표시 |

> ⚠️ **모드 ①과의 충돌** — [§2-3](#2-3--구멍--릴레이는-r-18영구-재식별자을-서버로-확대한다)의 **회전 RID는 "누가 누구에게"를 일부러 못 남기게** 만든 장치다.
> 기록을 남기려면 **안정 가명 ID가 필요**하다. 즉 **모드 ①(익명·무기록)과 모드 ②(기록)는
> 프라이버시 모델이 다르다** — 같은 서버가 두 모드를 겸할 수는 있어도, 사용자에게는
> **다른 약속**으로 보여야 한다.
...
### 5-9. ★ 그룹 팬아웃을 더 잘하는 법 — **구성원 재배포(swarm)**

사용자 질문: *"각 구성원과 1:1 세션을 열어 다중 송신 — 더 좋은 기법이 있나?"*
**있다.** 그리고 서버 없이도 된다.

**지금 방식의 한계** — 발신자가 같은 파일을 N번 **다시 암호화해서** N번 올린다.

**바꾸는 것 하나** — 파일을 **콘텐츠 키 `K`로 한 번만** 암호화한다(§3-8과 같은 구조).
그러면 **모든 구성원에게 가는 바이트가 동일해진다.** 그 순간 두 가지가 열린다:

| 열리는 것 | 왜 |
|---|---|
| **재배포(swarm)** | 이미 받은 구성원이 **아직 못 받은 구성원에게 전달**할 수 있다. 발신자 업링크가 O(N) → **O(log N)** |
| 재개·중복 제거 | 같은 바이트라 조각 단위 재개가 되고, 같은 파일을 여러 그룹에 보내도 blob 하나 |

> ★ **왜 안전한가** — 재배포되는 것은 **암호문뿐**이고, `K`는 **구성원별 봉투**로만 전달된다.
> 그리고 **구성원은 어차피 그 파일을 볼 자격이 있다.** 그러니 구성원끼리 암호문을 넘겨줘도
> **새로 새는 것이 없다.** 비구성원은 암호문을 얻어도 `K`가 없어 쓸모가 없다.
...
| 규칙 | 이유 |
|---|---|
| `blob_id = H(암호문)` — **내용 주소** | 위·변조 즉시 탐지 · 중복 제거 · 조각 재개의 기준 |
| 재배포는 **같은 roster 구성원 사이만** | 자격은 roster가 정한다(fail-closed) |
| 봉투는 **발신자만** 만든다 | 재배포자가 새 구성원에게 `K`를 넘기면 **자격 판정이 흩어진다** |
| 받은 쪽도 **무해화 게이트 통과** | [04 T-7](04-safe-transfer.md) — *보낸 사람이 누구든 믿지 않는다*. **구성원도 포함** |
| 재배포는 **옵트인·상한** | 남의 업로드를 대신 쓰는 것이라 사용자가 끌 수 있어야 한다 |

> ⚠️ **정직한 한계** — swarm은 **동시 접속 구성원이 있어야** 이득이다. 다들 다른 시간에
> 접속하는 조직에서는 효과가 없다. 그때 답은 서버(§5-10)다.
> 그리고 **LAN 멀티캐스트**는 검토했으나 **채택하지 않는다** — 신뢰성 재전송·혼잡 제어를
> 직접 만들어야 하고([06](06-network-stack.md) L3 제약), 이득은 같은 LAN·동시 접속에만 있다.
