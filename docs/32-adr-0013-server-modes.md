...
| **Q-32-13** | ★ **DR-9 개정 — 서버를 같은 저장소에 둘 것인가**([§9](#9-저장소-구성--한-저장소--배포-분리를-권한다-dr-9-개정-제안)) | 권장은 **한 저장소 + 배포 분리**. DR-9(별도 저장소)를 바꾸는 것이라 확정이 필요하다 |
| **Q-32-14** | 기록 등급 기본값과 **조직이 올릴 수 있는 상한**([§3-9](#3-9-기록-등급-l0l2--기본은-안전하게-필요하면-전부)) | L0 기본은 정했다. **L2를 제품이 허용할 것인가**(Q-32-4와 한 쌍) · 보존 상한은 얼마인가 |
| **Q-32-15** | ★ **UDP 세션 전송을 도입할 것인가**([§12-2](#12-2--홀펀칭-실현성-사용자-우선-질문--가능하되-선행-하나가-관문)) | 홀펀칭·릴레이의 **선행 관문**. 현행 TCP로는 홀펀칭 불안정 · 이걸 정하기 전엔 서버 네트워킹 착수 불가 |
...
> **개발 제외**(TODO만) · 명시 요청 시 착수. 아래는 "가장 단순한 완전한 것"의 범위.
