...
| [11 ADR-0004 수신 무해화](11-adr-0004-quarantine.md) | ✅ **Accepted**(08-08) — `.beepq` 레이아웃 · 위험 등급 4단계 확장자표 · 상태 기계(fail-closed) · MotW/quarantine API |
...
| [28 ADR-0011 설정 직렬화·영속 표준](28-adr-0011-settings-persistence.md) | 📐 **Proposed** — **Entry 레지스트리 = 저장 스키마**(설정 추가 = 1줄) · ★ **호스트 주입 시계 기반 coalescing 저장**(변경 N회 → 쓰기 1회 · starvation 상한) · 미지 키 보존 · 원자적 쓰기(fsync) · **다른 프로젝트로 분리 가능** |
...
| **21** | **신원 규격 — `PeerId`·`UserId` 생성과 유일성** | ✅ (🔴 §5 복제 대응은 확정 대기) |
...
| **26** | **실행·수동 테스트 가이드**(실행 모드·시나리오·Docker) | ✅ |
| **27** | **타입어헤드 한글 직접 조합 — IME 탈피의 기록**(두벌식 오토마타) | ✅ |
| **26** | 실행·수동 테스트 가이드 | ✅ |
| **27** | 타입어헤드 한글 조합 | ✅ |
| **28** | **ADR-0011 설정 직렬화·영속 표준(재사용 모듈)** | 📐 Proposed |
| **29~** | *(미배정)* | 신규 문서는 여기부터 |
