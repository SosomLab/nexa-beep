...
> **상태: 📐 Proposed** (2026-08-09) · 🔴 **사용자 확정 대기** · 결정 원장 **DR-27**
> **사용자 요구(2026-08-09)**:
...
| # | 항목 | 시점 |
|---|---|---|
| **Q-28-1** | 🔴 **크레이트 이름과 위치** — 이 저장소 안 `crates/nexa-conf`로 두고 나중에 분리할 것인가, 처음부터 별도 저장소인가([DR-9](10-decision-record.md) 선례) | 확정 시 |
...
> **다음 단계**: 🔴 사용자 확정 → Accepted 시 ⓐ **M3-11 설정 화면의 `Entry` 레지스트리에 저장을 태운다**(값 영속 — 현재 미구현) ⓑ `SaveScheduler`는 **`Debouncer` 확장**으로(신규 개념 없음) ⓒ 종료 flush는 **M1-8x 훅 재사용** ⓓ [Q-28-1](#9-열린-질문) 확정 후 크레이트 분리.
