...
> ### ⚡ 한 번에 하려면 — `tools/relaunch.sh`
>
> ```bash
> ./tools/relaunch.sh          # 종료 → 재빌드 → 3신원 기동 → 발견 확인
> ./tools/relaunch.sh --fresh  # A·B 신원을 새로(⚠️ 핀·그룹·프로필이 사라진다)
> ./tools/relaunch.sh --gate   # 빌드 대신 전체 게이트(fmt·clippy·test·rustdoc)
> ```
>
> **기본은 신원 보존**이다 — 폴더를 지우면 진행 중이던 그룹 테스트를 처음부터 다시 해야 한다.
> 아래는 그 스크립트가 하는 일을 손으로 푼 것이다(원리를 알아야 막혔을 때 고친다).
...
### T-7. 파일 전송 (그룹) — ★ 08-24 M5-1h 개편 반영

- 방에서 파일을 드롭한다 → 전송은 여전히 **구성원마다 1:1 세션 팬아웃**이지만,
  화면은 그룹창 축이다:
  - **발신(O 창)** — 그룹 스레드에 파일 풍선 **1개**, 본문 = `완료 n · 진행 n · 대기 n`
    (정지·실패·제외는 발생 시만). 개별 진행률·배너는 없다(설계 — 사용자 확정).
  - **수신(M1/M2 창)** — 승인 카드에 **"그룹" 행**(방 이름), 수신 라인은 **그 방의
    그룹 스레드**에 발신자 라벨과 함께 선다(1:1 창이 아니라). ⏸▶✕도 그 라인에서.
- **✕(제외) 확인** — 발신 풍선의 ✕ 하나가 유일한 제어다: 대기 구성원 = 철회 ·
  진행 중 구성원 = 취소(수신측 라인 "상대가 취소" 실패) · **완료 구성원 = 불변**.
- **확인** — 각 수신측에서 **무해화 게이트**를 거치는가(`.beepq` 격리 → 승인 → 실체화)
- **폴백 확인**(혼재 시) — 구버전 수신자는 그룹 표식(Control 18)을 무시하고
  종전처럼 1:1 창으로 받는다(자연 강등 · 전방 호환).
- ⚠️ **크기 주의** — 지금도 **N배 업로드**다(30명·10MB = 300MB). 개선안은
