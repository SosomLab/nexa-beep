...
### 3-2. 연결 수명·정리 기준 — "오래 안 붙는 대상"은 따로 관리하지 않는다 (09-05 실코드 점검)

> 질문: *"일정 시간 이상 연결이 되지 않는 대상은 제외되는가?"* → **서버에는 그런 명부가 없다.**
> 서버가 아는 "대상"은 **지금 살아 있는 TCP 제어 연결**뿐이다(저장 0 · [32 §2-3] 비기록 원칙).
> 연결이 끊기면 그 연결이 등록한 RID·토큰·채널·공개 목록이 **즉시** 회수되고, 끊기지 않은 연결은
> 얼마나 오래 조용하든 남는다. "제외"는 시간이 아니라 **연결의 죽음**이 기준이다.

| 단계 | 기준 | 어디 |
|---|---|---|
| **미등록 연결** | 접속 후 **10초** 안에 `Register`가 없으면 절단(핸드셰이크 read 타임아웃도 같은 10초) — 자리만 차지하는 침묵 연결 방지 | `nexa-beepd/src/lib.rs` `HANDSHAKE_TIMEOUT` |
| **등록 후 유휴** | **앱 계층 유휴 타임아웃 없음.** 클라이언트의 `Ping`(20초 주기)은 NAT 매핑 유지용이고 서버는 `Pong`만 돌려준다(마지막 수신 시각을 기록하지 않는다) | `nbeep-relay` 클라 액터 · beepd `C2s::Ping` |
| **죽음 감지**(FIN 없는 소멸 — 전원 차단·망 단절·NAT 만료) | **TCP keepalive**(유휴 10s → 5s 간격 · Linux/mac 재시도 3 = **≈25초**, Windows는 OS 기본 ≈1분) + **쓰기 타임아웃 30s**(상대가 안 읽어 버퍼가 찬 경우) → read/write `Err` → 연결 스레드 종료 | `nbeep-net/src/tcp.rs` `TcpLink::new`(accept된 스트림에도 적용) |
| **느린 소비자** | 그 연결로 갈 큐가 **8MiB**를 넘으면 kill 플래그 → 절단(무한 버퍼 대신 정직한 절단 — 쓰기 교착·메모리 폭주 회피) | beepd `QUEUE_CAP`·`enqueue` |
| **채널 상한** | 연결당 동시 채널 **64** 초과 `Open` = `status 2` 거절 | beepd `MAX_CHANNELS_PER_CONN` |
| **종료 시 회수** | RID·UDP 토큰 `retain` · 내 채널 전부 닫고 상대에 `ChClosed` · `conns` 제거 · 공개 목록 제거 + `PeerDown` 방송 — **전부 즉시·같은 스레드** | beepd `conn_thread` 말미 |
| **재접속 잔재** | `Register`는 **최신 등록 우선**(같은 RID를 새 conn이 덮어씀) — 아직 안 죽은 옛 연결의 RID가 새 연결을 막지 않는다 | beepd `C2s::Register` |
| **감지 전 창**(≈25초) | 죽었지만 아직 감지 안 된 연결에 `Open`이 오면 `status 0` + `Incoming`이 그 큐에 쌓인다 → 죽음 감지 시 `ChClosed`로 열었던 쪽에 통지 · 클라 사다리는 자체 12초 상한으로 먼저 포기하고 다음 후보로 | — |

**운영자가 알아 둘 결론**

- **유령 등록의 최대 수명 ≈ keepalive 감지 시간**(Linux 서버 ≈25초). 그보다 오래 남는 "오래 안 붙는 대상"은 구조상 존재하지 않는다 — 재시작해도 잃는 것은 진행 중 채널뿐(§3).
- 시간 기준 만료가 있는 것은 **클라이언트 쪽**이다 — 회전 RID(UTC 일 단위 · 어제/오늘/내일 셋 등록)와 GUI 목록의 roster `PeerDown` 처리. 서버는 시계를 신뢰 판정에 쓰지 않는다.
- 유휴 타임아웃을 **추가하지 않은 이유**: `Ping` 20초가 있으니 "N초 무수신 = 절단"을 넣을 수는 있으나, keepalive가 이미 같은 일을 커널에서 더 정확히 한다(전송 계층이 죽음을 아는데 앱이 추정할 이유가 없다 — 06 §7 "추정 금지"). 필요해지면 `Ping` 수신 시각 1개 필드로 끝나는 변경이고 와이어는 그대로다.
