...
**현황(09-17 1차)** — ★ **winget 첫 실게시** = `SosomLab.NexaBeep.Portable` 0.2.14([#427126](https://github.com/microsoft/winget-pkgs/pull/427126)) **09-11 병합 + publish 파이프라인 성공**(인덱스 `manifests/s/SosomLab/NexaBeep/Portable` 실재 → `winget install SosomLab.NexaBeep.Portable`). 설치본 [#427125](https://github.com/microsoft/winget-pkgs/pull/427125)은 OPEN — 09-11 자동 검증도 `STATUS_DLL_NOT_FOUND`로 실패했고, **원인은 검증기가 아니라 우리 쪽이다**: 릴리스 exe 3종이 **`vcruntime140.dll`**(VC++ 재배포 · 인박스 아님)에 동적 링크돼 있다(→ **TODO M5-4g** · [05 NFR-B-5] 위반 · 고치기 전엔 재검증해도 같다). beepd [#422579](https://github.com/microsoft/winget-pkgs/pull/422579) = `Validation-Completed` 승인 대기(26일). ★ **choco = 검수 요구가 2건이었다** — `iconUrl`(09-05 수정) 외에 **"description 등을 영어로"**(beepd 09-02 · beep 09-04)가 남아 있었다 → **nuspec 3종 + `chocolatey{install,uninstall}.ps1` 6종 영문화**(`1518c99` · 주석·`Write-Host`까지 · 동작 무변경) → 재제출 → **3종 전부 "(Maintainer updated, waiting for Reviewer)"**. ★ **재제출은 같은 버전으로 한다** — 미승인 버전이 걸려 있으면 **새 버전 push가 403**이다(실측: 클라 v0.2.15 403 → v0.2.2 ✓ · beepd 0.2.5 ✓). 스위치는 원복(`CHOCO_PUSH=false` · `BEEPD_WINGET_PUBLISH=true`).
