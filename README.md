...
| **릴레이 경유**(선택) | 특정 서버를 추가하면 IP·로컬에 매이지 않고 일반 메신저처럼 통신. 서버는 봉투만 보고 내용은 못 읽는다 | **동작·실증**(v0.2.6) — Managed + Test 1회로 **공식 기본 서버**(`beepd.sosomlab.com`) 등록 · **공개 사용자 자동 목록**(옵트인 roster) · 랑데부→홀펀칭→릴레이 폴백 · ★**실 NAT 실증**(집 맥 ↔ 회사 윈도우 연결 성공 08-22) · 원리 = [docs/42](docs/42-relay-rendezvous-walkthrough.md) |

## 현재 상태 (2026-09-07 · **v0.2.15 공개 — 다중 기기 신원 ADR-0015 S0~S2(핸들+페어링 암호 · 형제 세션 · 키 동기 · 내 기기 배지 · 키 교체) · 글꼴 폴백 체인 · 2-PC 릴레이 결함 수정 · 목록 증발 수정 · 비밀 파일 0600**)
...
| **배포** | **v0.2.16** — Windows(x64·ARM64) · macOS(x64·ARM64) · Linux(x64) **5타깃 × 2채널**(설치본·포터블 · 자산 14종) · **Homebrew** 자동 추종 · **winget = 포터블 게시됨**(`winget install SosomLab.NexaBeep.Portable` · 0.2.14 · 09-11 병합) · **0.2.16 재제출 검수 중**(#436462·#436463 — 0.2.14는 VC++ 재배포 의존 결함이 있어 닫았다 · M5-4g ✅) · **Chocolatey 3종 = 영문화 재제출 후 검수 대기**(통과 전 새 버전 제출 보류) · 서버 `nexa-beepd`는 **`beepd-v*` 별도 릴리스**(Linux = musl 정적) · 태그 push = 자동 → [Releases](https://github.com/SosomLab/nexa-beep/releases) |
| **테스트** | 워크스페이스 **807 green**(mac 09-05 · Win 797+회귀 7) · clippy 경고 0 · CI 4잡(lint / test 3-OS / cross-build / 예산 게이트) |
...
| **서버 축** | **UDP + 릴레이 서버 개통**(08-21~22) — 자작 경량 ARQ `UdpLink` · 릴레이 서버 MVP `nexa-beepd`(blind 파이프 · 외부 의존 0 · 0.35MB) · 랑데부→**홀펀칭**→릴레이 폴백 사다리 · **GUI Managed 배선 완료**(설정 SSOT·TOFU 핀·연결 테스트·검증 표시) · **공식 기본 서버 상주**(OCI · [docs/41](docs/41-beepd-ops-guide.md)) — 종단 Noise는 코드 무변경 관통 · 잔여 = 실 NAT 펀칭 실측 |
