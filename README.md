...
## 현재 상태 (2026-09-03 · **v0.2.14 공개 — 목록 증발 결함 수정(09-03) · Linux 첫 실기 완주(.deb 런처·시스템 테마·트레이 포커스·자동 실행 인스턴스별 슬롯) · 클립보드 3-OS 자체 구현**)
...
| **배포** | **v0.2.14** — Windows(x64·ARM64) · macOS(x64·ARM64) · Linux(x64) **5타깃 × 2채널**(설치본·포터블 · 자산 14종) · **Homebrew** 자동 추종 · winget·Chocolatey는 **검수 중**(winget v0.2.14 PR #427125/#427126 수동 리뷰 · choco 0.2.2 모더레이션 · 통과 전 새 버전 제출 보류) · 서버 `nexa-beepd`는 **`beepd-v*` 별도 릴리스**(Linux = musl 정적) · 태그 push = 자동 → [Releases](https://github.com/SosomLab/nexa-beep/releases) |
| **테스트** | 워크스페이스 **797 green**(Win Â· mac 799) · clippy 경고 0 · CI 4잡(lint / test 3-OS / cross-build / 예산 게이트) |
